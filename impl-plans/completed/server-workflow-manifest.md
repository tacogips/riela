...
Implement and review `riela serve` workflow manifest support plus the
...
resolution from the current directory with `RIELA_WORKFLOW_MANIFEST_ROOT`
...
can also come from `--workflow-manifest` or `RIELA_WORKFLOW_MANIFEST`.
...
#### packages/riela/src/workflow/manifest.ts
...
- [x] Resolve relative paths from current directory by default, with
      `RIELA_WORKFLOW_MANIFEST_ROOT` override support.
...
#### packages/riela/src/workflow/catalog.ts
#### packages/riela/src/workflow/load.ts
#### packages/riela/src/workflow/types.ts
#### packages/riela/src/workflow/overview.ts
...
#### packages/riela/src/cli/storage-and-options.ts
#### packages/riela/src/server/serve.ts
#### packages/riela/src/server/api.ts
...
- [x] Parse `--workflow-manifest <path>` for `riela serve`.
- [x] Read `RIELA_WORKFLOW_MANIFEST` as the environment fallback.
...
#### packages/riela/src/graphql/types.ts
#### packages/riela/src/graphql/schema/llm-run-overrides.ts
#### packages/riela/src/graphql/schema/execution-resolvers.ts
#### packages/riela/src/server/graphql-executable-schema.ts
...
#### packages/riela/src/lib-workflow-run-options.ts
#### packages/riela/src/workflow/auto-improve-policy.ts
#### packages/riela/src/workflow/engine/auto-improve-and-runner.ts
#### packages/riela/src/workflow/working-directory.ts
...
#### packages/riela/src/server/browser-overview.ts
#### packages/riela/src/server/graphql-schema-text.ts
#### README.md
...
- [x] Document `--workflow-manifest`, `RIELA_WORKFLOW_MANIFEST`, path rules, precedence, and startup validation.
...
|--------|-----------|--------|-------|
| Manifest types and loader | `packages/riela/src/workflow/manifest.ts` | COMPLETED | Covered in `packages/riela/src/workflow/manifest.test.ts` |
| Catalog source model | `packages/riela/src/workflow/catalog.ts`, `packages/riela/src/workflow/load.ts`, `packages/riela/src/workflow/overview.ts` | COMPLETED | Covered in workflow manifest/catalog tests and GraphQL overview tests |
| CLI and server startup | `packages/riela/src/cli.ts`, `packages/riela/src/cli/storage-and-options.ts`, `packages/riela/src/server/serve.ts`, `packages/riela/src/server/api.ts` | COMPLETED | Covered in targeted CLI serve tests |
| GraphQL allowlist enforcement | `packages/riela/src/graphql/types.ts`, `packages/riela/src/graphql/schema/llm-run-overrides.ts`, `packages/riela/src/graphql/schema/execution-resolvers.ts`, `packages/riela/src/server/graphql-executable-schema.ts` | COMPLETED | Covered in targeted GraphQL catalog/execution tests |
| Execution defaults and identity | `packages/riela/src/lib-workflow-run-options.ts`, `packages/riela/src/workflow/auto-improve-policy.ts`, `packages/riela/src/workflow/engine/auto-improve-and-runner.ts`, `packages/riela/src/workflow/working-directory.ts` | COMPLETED | Covered by manifest/catalog tests, GraphQL execution tests, and typecheck |
| Browser overview and docs | `packages/riela/src/server/browser-overview.ts`, `packages/riela/src/server/graphql-schema-text.ts`, `README.md` | COMPLETED | Covered by schema text update, overview model changes, and diff checks |
...
**Status**: COMPLETED
**Parallelizable**: No
**Deliverables**: `packages/riela/src/workflow/manifest.ts`,
`packages/riela/src/workflow/manifest.test.ts`
**Dependencies**: None
...
- [x] Path resolution is deterministic and relative paths resolve from the current directory by default, with `RIELA_WORKFLOW_MANIFEST_ROOT` override support.
...
**Deliverables**: `packages/riela/src/workflow/catalog.ts`,
`packages/riela/src/workflow/load.ts`, `packages/riela/src/workflow/types.ts`,
`packages/riela/src/workflow/overview.ts`
...
**Status**: COMPLETED
**Parallelizable**: Yes, after TASK-001
**Deliverables**: `packages/riela/src/cli.ts`,
`packages/riela/src/cli/storage-and-options.ts`,
`packages/riela/src/server/serve.ts`, `packages/riela/src/server/api.ts`
**Dependencies**: TASK-001

**Completion Criteria**:

- [x] `--workflow-manifest` and `RIELA_WORKFLOW_MANIFEST` are accepted for `serve`.
...
**Status**: COMPLETED
**Parallelizable**: No
**Deliverables**: `packages/riela/src/graphql/types.ts`,
`packages/riela/src/graphql/schema/llm-run-overrides.ts`,
`packages/riela/src/graphql/schema/execution-resolvers.ts`,
`packages/riela/src/server/graphql-executable-schema.ts`
...
**Deliverables**: `packages/riela/src/lib-workflow-run-options.ts`,
`packages/riela/src/workflow/auto-improve-policy.ts`,
`packages/riela/src/workflow/engine/auto-improve-and-runner.ts`,
`packages/riela/src/workflow/working-directory.ts`
...
**Status**: COMPLETED
**Parallelizable**: Yes, after TASK-002
**Deliverables**: `packages/riela/src/server/browser-overview.ts`,
`packages/riela/src/server/graphql-schema-text.ts`, `README.md`
**Dependencies**: TASK-002
...
- [x] Confirm `RIELA_WORKFLOW_MANIFEST_ROOT` overrides the base directory for both `workflowDirectory.relative` and `cwd.relative`.
...
**Status**: COMPLETED
**Parallelizable**: No
**Deliverables**: `packages/riela/src/cli/workflow-command-handler.ts`,
`packages/riela/src/cli/storage-and-options.ts`,
`packages/riela/src/cli/input-output-helpers.ts`,
`packages/riela/src/cli/workflow-manifest-validation.ts`,
`packages/riela/src/cli.test.ts`,
`packages/riela/src/server/serve.ts`,
`packages/riela/src/workflow/catalog.ts`,
`packages/riela/src/workflow/catalog.test.ts`,
...
- [x] Confirm path source precedence is positional argument, `--workflow-manifest`, then `RIELA_WORKFLOW_MANIFEST`.
...
- [x] Confirm server and catalog tests cover current-directory root, `RIELA_WORKFLOW_MANIFEST_ROOT`, disabled/unlisted rejection, and no-manifest fallback behavior.
...
- [x] Keep README aligned with `RIELA_WORKFLOW_MANIFEST_ROOT`, current-directory default relative root, and `workflow manifest validate` usage.
...
- `bun test packages/riela/src/workflow/manifest.test.ts packages/riela/src/workflow/catalog.test.ts packages/riela/src/workflow/overview.test.ts`
- `bun test packages/riela/src/cli.test.ts -t "workflow manifest validate|serve command|workflow list ignores"`
- `bun test packages/riela/src/server/serve.test.ts -t "manifest|overview"`
- `bun test packages/riela/src/server/api.test.ts`
- `bun test packages/riela/src/graphql/schema.test.ts packages/riela/src/server/graphql-queries-and-inspection.test.ts -t "workflowCatalogOverview|manifest|autoImprove"`
- `bun test packages/riela/src/workflow/auto-improve-policy.test.ts packages/riela/src/workflow/working-directory.test.ts`
- `bun run typecheck`
- `bun run lint:biome`
- `git diff --check`
...
- [x] `riela serve --workflow-manifest <path>` loads manifest version `1`.
- [x] `RIELA_WORKFLOW_MANIFEST` works when the flag is absent.
...
- [x] Current review confirms or fixes manifest relative root behavior:
      current directory by default and `RIELA_WORKFLOW_MANIFEST_ROOT` override.
...
`bun test packages/riela/src/workflow/manifest.test.ts packages/riela/src/workflow/catalog.test.ts`;
`bun test packages/riela/src/cli.test.ts -t "serve command"`;
`bun test packages/riela/src/graphql/schema.test.ts -t "workflowCatalogOverview|executeWorkflow"`;
...
`RIELA_WORKFLOW_MANIFEST` is present, and adding manifest-backed GraphQL and
...
`bun test packages/riela/src/workflow/manifest.test.ts packages/riela/src/workflow/catalog.test.ts`;
`bun test packages/riela/src/cli.test.ts -t "workflow list ignores|serve command"`;
`bun test packages/riela/src/graphql/schema.test.ts -t "manifest|workflowCatalogOverview|executeWorkflow"`;
`bun test packages/riela/src/server/serve.test.ts -t "manifest|overview"`;
...
directory by default, added `RIELA_WORKFLOW_MANIFEST_ROOT` for explicit
...
`RIELA_WORKFLOW_MANIFEST_ROOT` override, disabled-entry validation, ordinary
...
usage with `--workflow-manifest` and `RIELA_WORKFLOW_MANIFEST`. Verification
passed: `bun test packages/riela/src/workflow/manifest.test.ts packages/riela/src/workflow/catalog.test.ts packages/riela/src/workflow/overview.test.ts`;
`bun test packages/riela/src/cli.test.ts -t "workflow manifest validate|serve command|workflow list ignores"`;
`bun test packages/riela/src/server/serve.test.ts -t "manifest|overview"`;
`bun test packages/riela/src/server/api.test.ts`;
`bun test packages/riela/src/graphql/schema.test.ts packages/riela/src/server/graphql-queries-and-inspection.test.ts -t "workflowCatalogOverview|manifest|autoImprove"`;
