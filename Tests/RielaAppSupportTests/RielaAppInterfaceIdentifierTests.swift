...
final class RielaAppInterfaceIdentifierTests: XCTestCase {
  ...
  func testInstanceListUsesStateAndAddInstanceFlowInsteadOfEnabledDisabledSplit() throws {
    ...
    XCTAssertTrue(controllerSource.contains("width: 760"))
    XCTAssertTrue(controllerSource.contains("window.minSize = NSSize(width: 640"))
    ...
    XCTAssertTrue(controllerSource.contains("toolbar.heightAnchor.constraint(equalToConstant: 28)"))
    ...
    XCTAssertTrue(controllerSource.contains("promptForWorkflowSourceOption("))
    XCTAssertTrue(controllerSource.contains("promptForInstanceParameters(sourceOption:"))
    XCTAssertTrue(controllerSource.contains("WorkflowSourceSelection"))
    XCTAssertTrue(controllerSource.contains("case .retry:"))
    XCTAssertTrue(controllerSource.contains("Select a workflow source."))
    XCTAssertTrue(controllerSource.contains("Enter instance parameters."))
    XCTAssertTrue(controllerSource.contains("Select Workflow"))
    XCTAssertTrue(controllerSource.contains("Configure Instance"))
    XCTAssertTrue(controllerSource.contains("alert.addButton(withTitle: \"Next\")"))
    XCTAssertTrue(controllerSource.contains("addInstanceValueRow("))
    ...
    XCTAssertTrue(controllerSource.contains("NSButton(title: \"Instances\""))
    XCTAssertTrue(controllerSource.contains("NSImage(systemSymbolName: \"chevron.left\""))
    XCTAssertTrue(controllerSource.contains("backToInstancesButton.setAccessibilityLabel(\"Back to Instances\")"))
