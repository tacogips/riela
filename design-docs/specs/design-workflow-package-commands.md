...
Feature-local design for the user-facing riela package command surface that
ties registry registration, package search, package install/list/remove,
publish, refresh, and metadata inspection together.

## Overview

Riela packages are installable bundles published through GitHub-backed
...
`https://github.com/tacogips/riela-packages`. The developer-local checkout for
that registry is `<repo-root>-packages`. Users may add
personal registries, with registry configuration persisted under `~/.riela`.
...
`workflow checkout <package-id>`, or top-level `riela publish`.
...
```bash
riela package install <package-id> [--registry <registry-url-or-alias>] [--branch <branch>] [--user-scope] [--workflow-definition-dir <path>] [--overwrite] [--yes] [--output json|text]
riela package search [query] [--registry <registry-url-or-alias>] [--tag <tag>] [--backend <backend>] [--limit <n>] [--refresh] [--output table|json|text]
riela package registry add <id> --registry-url <url> [--local-path <path>] [--branch <branch>] [--output json|text]
riela package registry list [--output json|text]
...
riela package publish <workflow-name-or-path> [--registry <registry-url>] [--registry-local-path <path>] [--branch <branch>] [--package-id <id>] [--source-workflow-dir <path>] [--message <text>] [--create-pr] [--pr-base <branch>] [--dry-run] [--output json|text]
riela workflow checkout <github-directory-url> [--user-scope] [--overwrite] [--output json|text]
riela workflow run <package-id-or-github-directory-url> --from-registry [--registry <registry-url-or-alias>] [--branch <branch>] [--output json|text]
```
...
`~/.riela`, and optionally records an alias, default branch, and local path.
...
- `--user-scope` installs package workflows under `~/.riela/workflows` and
  package-managed skill data under `~/.riela-managed`.
- Node package installs write package-owned add-on artifacts under
  `<project>/.riela/addons` or `~/.riela/addons` and include
...
`riela workflow usage <workflow-name> --scope <scope>` and a package install
hint such as `riela package install <package-id> --user-scope` only when the
...
through `riela package install <package-id>`.

Required checkout decisions:

- project scope is the default installation destination
- `--user-scope` installs under `~/.riela/workflows`
...
`riela package publish` publishes one validated workflow directory into a GitHub
...
- The canonical publish command is `riela package publish`.
...
- Personal registry configuration is stored under `~/.riela`, not project
  scope.
...
- Whether `riela workflow publish` should be implemented as a documented
