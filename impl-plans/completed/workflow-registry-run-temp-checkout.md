...
- Allow raw GitHub directory URLs without `riela-package.json` only after
...
- Preserve retained temporary checkout cleanup for local `session resume` and
  `session continue`.
- Refresh tests, README, and `.agents/skills/riela-workflow-run/SKILL.md`.
...
This plan changes riela CLI/package-resolution behavior before the existing
...
### `packages/riela/src/workflow/checkout/github-directory.ts`
...
- `packages/riela/src/cli/workflow-run-command.ts`
- `packages/riela/src/cli/registry-run-provenance.ts`
- `packages/riela/src/cli/session-command-handler.ts`
...
- `README.md`
- `.agents/skills/riela-workflow-run/SKILL.md`
...
- TASK-002 and TASK-003 are conceptually separable but both write
  `packages/riela/src/workflow/packages/temp-run.ts`; run them sequentially
...
- `bun run packages/riela/src/bin.ts workflow run <package-id> --from-registry --registry default --output json`
- `bun run packages/riela/src/bin.ts workflow run https://github.com/user/workflow-repo/tree/main/workflow-dir --from-registry --output json`
- `bun run packages/riela/src/bin.ts workflow run https://github.com/user/workflow-repo/workflow-dir --from-registry --branch main --output json`
- `bun run packages/riela/src/bin.ts workflow run user/workflow-dir --from-registry --registry default --output json`
- `bun run packages/riela/src/bin.ts workflow run user/workflow-dir --from-registry --endpoint http://127.0.0.1:43173/graphql`
- `bun run typecheck`
- `bun run lint:biome`
- `git diff --check`
