...
- `packages/riela/src/events/adapters/matrix.ts`: Matrix `/sync`
  ownership, normalized `chat.message` shape, room/thread/reply mapping, and
  diagnostic sink behavior.
- `packages/riela/src/events/adapters/matrix-attachments.ts`: Matrix
...
- `packages/riela/src/events/validate-source-matrix.ts`: Matrix source
  validation for attachment option shape and byte bounds.
- `packages/riela/src/events/adapters/matrix.test.ts` and
  `packages/riela/src/events/config.test.ts`: mocked sync/download and
...
the design treats Codex-reference files as local riela behavior references,
