...
- `design-docs/specs/design-workflow-package-commands.md`
- `design-docs/specs/design-node-addon-catalog-and-chat-reply-worker.md`
- `packages/riela/src/workflow/packages/`
- `packages/riela/src/workflow/node-addons.ts`
- `packages/riela/src/workflow/local-node-addons.ts`
- `packages/riela/src/workflow/addon-package-boundary.ts`
...
- Extend `riela-package.json` with package kind support where omitted `kind`
...
- [x] Reject absolute paths, parent traversal, symlink escapes, duplicate
      `(name, version)` entries, empty `addons[]`, and `riela/*` names.
...
- [x] Keep `riela package install <package-id>` as the canonical install
...
- [x] Add tests proving package-provided `riela/*` add-ons cannot shadow
      built-ins.
...
- [x] Add fixtures for valid node-addon packages, unsafe source paths,
      duplicate add-ons, `riela/*` shadow attempts, executable package
...
| --- | --- | --- | --- |
| Package kind and manifest types | `packages/riela/src/workflow/packages/types.ts`, `packages/riela/src/workflow/packages/manifest.ts` | Completed | `packages/riela/src/workflow/packages/packages.test.ts` |
| Search/index kind support | `packages/riela/src/workflow/packages/index.ts`, `packages/riela/src/workflow/packages/search.ts` | Completed | `packages/riela/src/workflow/packages/packages.test.ts`, `packages/riela/src/cli.test.ts` |
| Node add-on artifact validation | `packages/riela/src/workflow/packages/node-addon-install.ts`, `packages/riela/src/workflow/addon-package-boundary.ts` | Completed | `packages/riela/src/workflow/addon-package-boundary.test.ts`, `packages/riela/src/workflow/packages/packages.test.ts` |
| Node add-on projection and records | `packages/riela/src/workflow/packages/checkout.ts`, `packages/riela/src/workflow/packages/checkout-node-addon.ts`, `packages/riela/src/workflow/packages/checkout-records.ts`, `packages/riela/src/workflow/packages/status.ts` | Completed | `packages/riela/src/workflow/packages/packages.test.ts` |
| CLI and docs | `packages/riela/src/cli/workflow-package-command-handler.ts`, `packages/riela/src/cli/argument-parser.ts`, `packages/riela/src/cli/run-cli.ts`, `README.md` | Completed | `packages/riela/src/cli.test.ts` |
| Authored workflow validation | `packages/riela/src/workflow/local-node-addons.ts`, `packages/riela/src/workflow/node-addons.ts` | Completed | `packages/riela/src/workflow/authored-workflow.test.ts`, `packages/riela/src/workflow/packages/packages.test.ts` |
...
- [x] Missing node packages are not fetched during workflow load, validation, or
      execution.
- [x] `riela/*` package-provided add-ons and executable plugin behavior are
      rejected.
...
bun test packages/riela/src/workflow/addon-package-boundary.test.ts
bun test packages/riela/src/workflow/authored-workflow.test.ts
bun test packages/riela/src/cli.test.ts
bun run packages/riela/src/bin.ts package search --output json
bun run packages/riela/src/bin.ts package install <node-package-id> --output json
bun run packages/riela/src/bin.ts package list --output json
bun run tsc --noEmit
git diff --check
```
...
`bun run lint:biome`,
`bun run typecheck`,
`bun test packages/riela/src/workflow/packages/packages.test.ts packages/riela/src/workflow/addon-package-boundary.test.ts packages/riela/src/workflow/authored-workflow.test.ts packages/riela/src/cli.test.ts`,
`bun packages/riela/src/bin.ts package search --output json`,
`bun packages/riela/src/bin.ts package search --kind node-addon --output json`,
`bun packages/riela/src/bin.ts package list --output json`, and
`git diff --check`.
