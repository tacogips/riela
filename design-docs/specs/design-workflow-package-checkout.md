...
riela package registries.

## Overview

The package install feature extends the existing scoped workflow checkout
implementation so `riela` can install package-selected workflow
...
with `https://github.com/tacogips/riela-packages` as the default public
...
- GitHub issue inspection: `https://github.com/tacogips/riela/issues/35`
...
- Support explicit user scope installation under `~/.riela/workflows`.
- Store user-scope managed skill data under `~/.riela-managed` before any
...
```text
~/.riela-managed/packages/<package-id>/<version-or-ref>/skills/<vendor>/...
```
...
```bash
riela package install <package-id> [--registry <registry-url-or-id>] [--branch <branch>] [--user-scope] [--workflow-definition-dir <path>] [--overwrite] [--yes] [--output json|text]
```

The workflow checkout command keeps the current direct URL form:

```bash
riela workflow checkout https://github.com/<owner>/<repo>/tree/<ref>/<workflow-dir>
```
...
- `--user-scope` installs under the configured user root, normally
  `~/.riela/workflows` for workflows and `~/.riela-managed` for managed
  skill data.
...
```bash
riela package list [--scope project|user|auto] [--workflow-definition-dir <path>] [--output json|text]
riela package status <package-id-or-workflow-name> [--scope project|user|auto] [--install-id <id>] [--workflow-definition-dir <path>] [--output json|text]
riela package remove <package-id-or-workflow-name> [--scope project|user|auto] [--install-id <id>] [--workflow-definition-dir <path>] [--output json|text]
```
...
- project managed skill root:
  `<project-root>/.riela/managed/packages/<package-id>/skills/`
...
- validate `riela-package.json`, checksum/integrity metadata, and the
  selected workflow bundle through the same validation path used by persistent
  package install when package metadata exists
- for direct GitHub directory URL targets without `riela-package.json`,
...
`~/.riela/workflow-registry/checkouts/`, must not mutate project or user
...
and must remain separate from `~/.riela/workflow-registry/checkouts/`.
...
`~/.riela-managed` without projecting to a vendor location, and `projected`
...
```text
~/.riela/workflow-registry/checkouts/<install-id>.json
```
...
`~/.riela/workflow-registry/checkouts/` and does not contact registries.
...
`riela workflow checkout`. Those records are intentionally not package-owned
...
`riela package status <name>` is a package-boundary diagnostic as well as an
...
`riela workflow usage <workflow-name> --scope <scope>`. If the user wants
package lifecycle commands such as list/status/update/remove, status should
explain that the workflow must be installed through `riela package install`
...
- Delete workflow-local `.riela-package-provenance.json` with the workflow
  directory.
...
- Add `riela package install` as the canonical package install command and
  reject workflow-scoped package checkout forms.
- Keep direct GitHub directory URL checkout on `riela workflow checkout`.
...
- Store user-scope managed skills below `~/.riela-managed` and treat vendor
  projection as a safe secondary copy.
- Store package checkout provenance under
  `~/.riela/workflow-registry/checkouts/`.
- Make `riela package install` the canonical package lifecycle command.
- Make `riela package list` a local catalog read that reports installed
...
- Make `riela package status <workflow>` return explanatory
  workflow-checkout status when no package record matches but a raw workflow
  checkout record exists.
- Make `riela package remove` a checkout-record-driven deletion operation
...
bun test packages/riela/src/workflow/packages/checkout.test.ts
bun test packages/riela/src/cli.test.ts
bun test packages/riela/src/cli.test.ts -t "package list"
bun test packages/riela/src/cli.test.ts -t "package remove"
bun test packages/riela/src/cli.test.ts -t "registry"
bun run typecheck
bun run packages/riela/src/bin.ts package list --output json
bun run packages/riela/src/bin.ts package remove --install-id <install-id> --output json
bun run packages/riela/src/bin.ts workflow run <package-id> --from-registry --mock-scenario <fixture> --output json
bunx biome check packages/riela/src/workflow/packages packages/riela/src/cli
