...
public enum CursorCLISessionSQLiteIndex {
  ...
  public static func openCursorCLIDb(cursorCLIHome: String? = nil) -> String? {
    let path = statePath(cursorCLIHome: cursorCLIHome)
  ...
  private static func rowToSession(_ row: [String: String]) -> CursorCLISession? {
    ...
    let git = [row["git_sha"], row["git_branch"], row["git_origin_url"]].contains { nonEmpty($0) != nil }
      ? CursorCLISessionGit(sha: nonEmpty(row["git_sha"]), branch: nonEmpty(row["git_branch"]), originURL: nonEmpty(row["git_origin_url"]))
      : nil
