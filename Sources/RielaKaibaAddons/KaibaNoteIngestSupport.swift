...
func notePageInputs(_ context: NoteAddonContext) throws -> [NotePageInput] {
  ...
  guard values.count <= context.maxPageCount else {
    throw noteAddonInvalidInput(
      "kaiba/notebook-ingest-pages pages has \(values.count) items; max \(context.maxPageCount)"
    )
  }
  return try values.enumerated().map { index, value in
    guard case let .object(page) = value else {
      throw noteAddonInvalidInput("kaiba/notebook-ingest-pages pages[\(index)] must be an object")
    }
    guard let body = nonEmptyString(page["bodyMarkdown"])
      ...
      throw noteAddonInvalidInput(
        "kaiba/notebook-ingest-pages pages[\(index)].bodyMarkdown is required"
      )
    }
