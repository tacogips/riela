...
export function createDesktopTransport(invoke: Invoke, endpoint = '', token: () => string = () => '') {
  return async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    return new Promise<Response>((resolve, reject) => {
      ...
      invoke<NativeResponse>(endpoint ? 'riela_remote_request' : 'riela_request', {
        ...(endpoint ? { endpoint } : {}),
        request: {
...
export function normalizeServerOrigin(value: string): string {
  ...
  if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password
    ...
    throw new TypeError('Enter a server origin such as https://riela.example (without a path or credentials).')
  }
  return url.origin
}

const endpointKey = 'riela.desktop.server-origin'
export function desktopServerOrigin(): string {
  return isDesktop() ? sessionStorage.getItem(endpointKey) ?? '' : ''
}

export function switchDesktopConnection(endpoint: string): void {
  if (endpoint) sessionStorage.setItem(endpointKey, normalizeServerOrigin(endpoint))
  else sessionStorage.removeItem(endpointKey)
  // Reload destroys old polling, pending UI operations, CSRF and profile state.
...
export function hasBrowserAccessToken(): boolean { return browserAccessToken !== '' }
...
export function rielaFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
  ...
  return native
    ? createDesktopTransport(native.core.invoke, desktopServerOrigin(), () => browserAccessToken)(input, init)
