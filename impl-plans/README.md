...
| ---- | --------: | ------ | ---------- |
| `active/agent-node-output-contract` | 6 | Planned 2026-09-21, no code written; agent-node execution contract (`design-docs/specs/design-agent-node-output-contract.md`): validate-time agentSandbox + output.jsonSchema requirements, no-contract text-wrap simplification, schema-default retry 2, typed template-resolution error naming producer/path/consumer; D4 bundle edits are a riela-packages follow-up work package | Agent-node output contract |
