...
function assertPrivateRuntimePath(filePath) {
  ...
  if (!isAllowed) {
    throw new Error(
      `RIELA_X_DIGEST_STATE_FILE must point to an ignored/private runtime path, got ${filePath}`,
    );
  }
}

const stateFile =
  process.env.RIELA_X_DIGEST_STATE_FILE ||
  ".riela-data/x-follower-ai-business-digest/state.json";
assertPrivateRuntimePath(stateFile);
const accountUsername =
  process.env.RIELA_X_DIGEST_ACCOUNT_USERNAME ||
  process.env.X_GW_ACCOUNT_USERNAME ||
  "@tacogips";
const lookbackMinutes = readPositiveInteger(
  "RIELA_X_DIGEST_LOOKBACK_MINUTES",
  60,
);
const maxPosts = Math.max(
  5,
  Math.min(readPositiveInteger("RIELA_X_DIGEST_MAX_POSTS", 50), 50),
);
