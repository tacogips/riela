...
PORT="${RIELA_MATRIX_SAMPLE_PORT:-18008}"
HOMESERVER_URL="http://127.0.0.1:${PORT}"
RUN_ROOT="${RIELA_MATRIX_SAMPLE_RUN_ROOT:-${REPO_ROOT}/tmp/matrix-chat-reply-sample}"
DATA_DIR="${RIELA_MATRIX_SAMPLE_DATA_DIR:-${RUN_ROOT}/synapse-data}"
EVENT_ROOT="${RUN_ROOT}/.riela-events"
ARTIFACT_ROOT="${RUN_ROOT}/artifacts"
REGISTRATION_SECRET="${RIELA_MATRIX_SAMPLE_REGISTRATION_SECRET:-riela-local-registration-secret}"
ALICE_PASSWORD="${RIELA_MATRIX_SAMPLE_ALICE_PASSWORD:-riela-alice-password}"
...
wait_for_listener() {
  ...
  for _ in $(seq 1 30); do
    ...
    if [[ -n "${listener_pid}" ]] && ! kill -0 "${listener_pid}" 2>/dev/null; then
      cat "${log_file}" >&2 || true
      echo "riela events serve exited before it was ready" >&2
      return 1
    fi
    sleep 1
  done
  ...
  cat "${log_file}" >&2 || true
  echo "riela events serve did not report readiness" >&2
  return 1
}
...
export RIELA_MATRIX_SAMPLE_DATA_DIR="${DATA_DIR}"
...
for _ in $(seq 1 90); do
  ...
  if [[ -n "${listener_pid}" ]] && ! kill -0 "${listener_pid}" 2>/dev/null; then
    ...
    echo "riela events serve exited before the Matrix reply was observed" >&2
    exit 1
  fi
  sleep 1
done
