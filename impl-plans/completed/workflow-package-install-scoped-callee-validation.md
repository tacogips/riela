...
- Reproduction command:
  `bun run packages/riela/src/bin.ts package install codex-impl-plan-completion-loop --pre-install-check --output json`
...
- `packages/riela/src/workflow/validate/semantic-validation-and-addons.ts`
...
| --- | --- | --- | --- |
| Install validation resolver | `packages/riela/src/workflow/packages/checkout.ts` | COMPLETED | Package checkout regression tests |
| Scoped catalog/load integration | `packages/riela/src/workflow/catalog.ts`, `packages/riela/src/workflow/load.ts` | NOT_STARTED | Loader/catalog tests if API changes |
| Cross-workflow diagnostics | `packages/riela/src/workflow/validate/semantic-validation-and-addons.ts` | COMPLETED | Missing-callee regression test |
| Package install tests | `packages/riela/src/workflow/packages/packages.test.ts` | COMPLETED | Focused install validation cases |
| CLI smoke path | `packages/riela/src/cli/workflow-package-command-handler.ts` | COMPLETED_WITH_ENV_BLOCKER | Manual reproduction command |
...
- `packages/riela/src/workflow/packages/checkout.ts`
- Optional scoped helper additions in `packages/riela/src/workflow/catalog.ts`
  or `packages/riela/src/workflow/load.ts` only if temporary-root composition
...
- `packages/riela/src/workflow/packages/checkout.ts`
- Optional `packages/riela/src/workflow/validate/semantic-validation-and-addons.ts`
...
- Run the issue reproduction command from `<repo-root>`
...
- TASK-001, TASK-002, TASK-004, and TASK-005 are sequential because they share
  `packages/riela/src/workflow/packages/checkout.ts` behavior and tests.
...
bun test packages/riela/src/workflow/load.test.ts packages/riela/src/workflow/validate.test.ts
bun run typecheck
bun run packages/riela/src/bin.ts package install codex-impl-plan-completion-loop --pre-install-check --output json
```
...
```bash
bun test packages/riela/src/workflow/catalog.test.ts packages/riela/src/workflow/runtime-readiness-backends.test.ts
```
