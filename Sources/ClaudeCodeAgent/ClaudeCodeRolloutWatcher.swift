...
public final class ClaudeCodeRolloutWatcher: @unchecked Sendable {
  ...
  public static func sessionsWatchDir(claudeCodeHome: String? = nil) -> String {
    URL(fileURLWithPath: claudeCodeHome ?? resolveClaudeCodeHome(), isDirectory: true).appendingPathComponent("sessions", isDirectory: true).path
  }
  ...
  public func flush() -> [ClaudeCodeRolloutWatcherEvent] {
    ...
    var events: [ClaudeCodeRolloutWatcherEvent] = []
      ...
      if let text = String(data: appended, encoding: .utf8) {
        ...
        if let lastNewline = text.lastIndex(of: "\n") {
          ...
          for rawLine in completeText.split(separator: "\n", omittingEmptySubsequences: true) {
            if let line = ClaudeCodeRolloutReader.parseRolloutLine(String(rawLine)) {
        ...
        if !trailing.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty, let line = ClaudeCodeRolloutReader.parseRolloutLine(trailing) {
          events.append(.line(path: path, line: line))
