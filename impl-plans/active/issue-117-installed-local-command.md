...
- branch: `fix/example-contract-migration`; baseline `b75dcfb25b2c386550dfd947773cd138e621f689`.
- progressPath: `impl-plans/progress/issue-117-installed-local-command.md`.
- evidenceRoot: `tmp/issue-117-local-command/`.

User intent: complete the installed local-command portion of #117. The baseline
handles native/container dependencies, but the original YouTube package uses
three local-command dependencies. Validate/inspect must accept a synthetic valid
...
| --- | --- |
| `Sources/RielaCLI/WorkflowInstalledAddonRequirements.swift` | Extend verified result with the matched descriptor and optional verified executable location. Count reference/version candidates before filtering execution-kind validity. Accept explicit local-command only after existing identity/lock/integrity checks and contained executable-file readiness. |
| `Sources/RielaCLI/WorkflowValidateInspectCommands.swift` | Project verified dependency executable requirements and availability from the same verified metadata contract. Preserve required environments and callee-local context; retain native/container and package-owned local-command behavior. |
| `Sources/RielaCLI/WorkflowAddonHostRequirements.swift` (only if needed) | Move the coherent host requirement/availability helper group from the 971-line command file if edits would exceed 1000 lines. No new abstraction or unrelated extraction. |
...
| `tmp/issue-117-local-command/issue-117-installed-local-command/` | Fixtures, immutable intent snapshots, hashes, drivers, complete logs and numeric status records; never stage. |
...
external-add-on guard; no Core or resolution change is expected. If a concrete
...
### LC-01: regression fixture and baseline — dependsOn []

Fresh-read design, source helpers, tests, and Swift skill. Create a filesystem
fixture using current manifest/checksum/lock-generation APIs. Use package ID
...
Run focused regression tests before production edits. Retain expected failing
baseline with diagnostic evidence showing the explicit valid local-command path
is unresolved. Record why adding a kind allowlist alone cannot supply executable
host requirements. Baseline failures are not passing implementation evidence.
Deliver tests/fixture, baseline logs, and causal trace in progress file.

### LC-02: verified matching and projection — dependsOn [LC-01]

Make the narrow edits in the ownership table. Verify reference uniqueness before
...
exact edits and reasons. Deliver production change and passing focused tests.
...
| --- | --- |
| Package ID, workflow name, direct package-directory | Same verified package/locks; successful validate and inspect; canonical executable paths retained for all three dependency references. |
...
Author `tmp/issue-117-local-command/verify.py` as a foreground
...
81/81 positive results, and negative CLI cases for missing kind, registry/digest
mismatch, missing/non-executable entrypoint, unknown/ambiguous dependency,
unrelated copy, and scope substitution. Restore/rebuild fixtures between mutations.
...
substitute for current source-tree evidence.
...
Never discard failures or overwrite previous attempts. Poll any tool session
...
../../../.build/debug/riela workflow validate youtube-flow --workflow-definition-dir .riela/packages/@issue117/youtube-flow --output json
../../../.build/debug/riela workflow inspect youtube-flow --workflow-definition-dir .riela/packages/@issue117/youtube-flow --output json
```
...
The owner creates its progress file on implementation start. Use a row for each
...
expected failing baseline, expected negative checks, and final passing gates.
