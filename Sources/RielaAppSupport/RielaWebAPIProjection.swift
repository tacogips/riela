...
public enum RielaWebHostKind: String, Sendable {
  case app = "riela-app"
  case server = "cli-serve"
}

/// Read-only web projections shared by the native host and the HTTP server.
/// The host supplies runtime state; projection code owns no listener or AppKit UI.
@MainActor
public struct RielaWebAPIProjection {
  private let hostKind: RielaWebHostKind
  private let daemonProfileName: RielaAppProfileName
  private let daemonState: RielaAppDaemonWorkflowState
  private let daemonInstances: [WorkflowInstance]
  private let daemonWorkflowSources: [RielaAppDaemonWorkflowCandidate]
  private let webRevision: Int
  private let webSessionStoreRootPath: String
  private let serverSettings: JSONObject
  ...
  public init(
    ...
    hostKind: RielaWebHostKind = .app
  ) {
    self.hostKind = hostKind
    daemonProfileName = profile
    daemonState = state
    daemonInstances = instances
    daemonWorkflowSources = sources
    webRevision = revision
    webSessionStoreRootPath = sessionStoreRoot
    self.serverSettings = serverSettings
    self.runtimeSnapshot = runtimeSnapshot
    self.environment = environment
  }

  public func response(for request: RielaHTTPRequest, csrfToken: String) -> RielaHTTPResponse {
    let components = request.percentEncodedPath.split(separator: "/").map(String.init)
    switch (request.method, request.path) {
    case ("GET", "/api/v1/bootstrap"):
      return webJSON([
        "apiVersion": .string("v1"),
        "hostKind": .string(hostKind.rawValue),
    ...
    default:
      return webParameterizedResponse(components: components, request: request)
    }
  }
            ...
            return .object([
              ...
              "transitions": .array(transitions.map { transition in
                ...
                return .object([
                  ...
                  "toWorkflowId": transition.toWorkflowId.map { .string(projection.identifier($0).value) } ?? .null,
