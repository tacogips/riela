...
Riela workflow packages are workflow bundle directories registered through Git
...
`https://github.com/tacogips/riela-packages`, with the local development
checkout expected at `<repo-root>-packages`.

The registry feature extends the existing scoped workflow catalog:

- project scope remains `<project>/.riela/workflows`
- user scope remains `~/.riela/workflows`
- personal registry configuration is persisted under `~/.riela`
...
- Primary source touchpoints:
  - `packages/riela/src/workflow/catalog.ts`
  - `packages/riela/src/workflow/usage.ts`
  - `packages/riela/src/workflow/runtime-db.ts`
...
`packages/riela/src/workflow/packages/` and expose small typed services that
...
- manifest: load and normalize `riela-package.json`
...
A registry entry represents a GitHub repository that riela can use for
...
`~/.riela/workflow-packages/registries.json`. This keeps registry state out of
project catalogs while allowing checkout records to remain in the existing
`~/.riela/workflow-registry/checkouts/` location for provenance.
...
parent directory by creating `~/.riela/workflow-packages/`. Invalid JSON or
...
Each workflow package has a manifest named `riela-package.json` at the
...
- normalized `riela-package.json` content with `checksum` and
...
The checksum written into `riela-package.json` is computed from the normalized
...
flattened registry view of one `riela-package.json` plus selected workflow
bundle metadata.
...
`riela-package.json`, load the referenced workflow bundle, validate it through
...
- explicit user scope installs under `~/.riela/workflows`
...
Persistent `riela package install` must therefore use a dependency-aware
install transaction:
...
fails after installing missing transitive dependencies, riela removes only
...
`packages/riela/src/workflow/packages/` with registry config, manifest,
...
After implementation, current-dir `.riela` workflow content should be moved
...
- Personal registries are persisted under `~/.riela`, not project scope.
...
- The `packages/riela/src/workflow/packages/` module is the feature-local
...
- Moving current-dir `.riela` content into the registry can disrupt local
