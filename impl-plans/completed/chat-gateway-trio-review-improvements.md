...
then make only low-risk improvements that keep provider complexity in riela
...
- Issue reference: GitHub PR #39,
  `https://github.com/tacogips/riela/pull/39`
...
#### packages/riela/src/events/adapters/telegram-gateway.ts
#### packages/riela/src/events/adapters/telegram-gateway-reply.ts
#### packages/riela/src/events/validate-source-telegram-gateway.ts
...
#### packages/riela-addons/src/native-node-executor/chat-and-gateway-addons.ts
#### packages/riela-addons/src/node-addons/chat-persona-router-config.ts
#### packages/riela-addons/src/node-addons/addon-payload-resolution.ts
#### packages/riela/src/workflow/chat-persona-router-types.ts
...
interface ChatPersonaRouterAuthoringExpectation {
  readonly addon: "riela/chat-persona-router";
...
interface ChatReplyWorkerAuthoringExpectation {
  readonly addon: "riela/chat-reply-worker";
...
#### examples/event-sources/.riela-events/
#### examples/README.md
...
interface ChatWorkflowAuthoringSurface {
  ...
  readonly providerSpecificConfigLocation: ".riela-events";
  readonly providerSpecificPromptLogicAllowed: false;
  readonly replyAddon: "riela/chat-reply-worker";
  readonly personaRouterAddon?: "riela/chat-persona-router";
}
```
...
- [x] Preserve intentional redaction fixtures in
      `packages/riela/src/events/adapters/telegram-gateway.test.ts` and
...
| Telegram boundary | `packages/riela/src/events/adapters/telegram-gateway*.ts`, `packages/riela/src/events/validate-source-telegram-gateway.ts` | COMPLETED | `telegram-gateway.test.ts`, events validation |
| Chat add-ons | `packages/riela-addons/src/**/chat*.ts`, `packages/riela/src/workflow/chat-persona-router-types.ts` | COMPLETED | `native-node-executor-addons-commands.test.ts` |
...
| Agent boundaries | `packages/riela-adapters/src/codex.ts`, persona node JSON | COMPLETED | `codex.test.ts` |
...
**Status**: Completed
**Parallelizable**: No
**Deliverables**: `packages/riela/src/events/adapters/telegram-gateway.ts`, `packages/riela/src/events/adapters/telegram-gateway-reply.ts`, `packages/riela/src/events/validate-source-telegram-gateway.ts`, related tests
...
**Status**: Completed
**Parallelizable**: No
**Deliverables**: `packages/riela-addons/src/native-node-executor/chat-and-gateway-addons.ts`, `packages/riela-addons/src/node-addons/chat-persona-router-config.ts`, `packages/riela-addons/src/node-addons/addon-payload-resolution.ts`, related tests
**Dependencies**: TASK-001
...
**Status**: Completed
**Parallelizable**: No
**Deliverables**: `packages/riela-adapters/src/codex.ts`, `packages/riela/src/workflow/adapters/codex.test.ts`, persona node JSON if needed
**Dependencies**: TASK-001
...
`packages/riela/src/events/adapters/telegram-gateway.test.ts` and
`packages/riela/src/events/adapters/matrix.test.ts`, and dummy-token
...
names such as `tokenEnv` and `RIELA_TELEGRAM_BOT_TOKEN` are allowed only when no
...
- [x] Telegram complexity is verified to live in riela adapters/validation
...
`packages/riela/src/events/adapters/telegram-gateway.ts` was 1092 lines and
needed the temporary `noExcessiveLinesPerFile` suppression despite the accepted
design requiring Telegram complexity to stay in riela-owned adapter code.
Fixed by extracting adapter-owned bounded history helpers to
`packages/riela/src/events/adapters/telegram-gateway-history.ts`, leaving
...
reply-worker example, `riela/chat-persona-router` and
`riela/chat-reply-worker` stay provider-neutral, and codex-agent/Cursor
