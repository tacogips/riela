...
const root = workflowMemoryRoot.length > 0
  ? workflowMemoryRoot
  : process.env.RIELA_TRIO_MEMORY_ROOT && process.env.RIELA_TRIO_MEMORY_ROOT.length > 0
    ? process.env.RIELA_TRIO_MEMORY_ROOT
    : "/tmp/riflow-tribot";
const personaId = safeSegment(process.env.RIELA_TRIO_MEMORY_PERSONA_ID, "persona");
const personaName = process.env.RIELA_TRIO_MEMORY_PERSONA_NAME || personaId;
