...
- The workflow validates as a step-addressed Slack Gateway chat bundle.
...
- Each persona can return `memoryEntries` for explicit remember requests,
  ...
  persona-scoped tags.
- Slack replies are sent through `riela/chat-reply-worker` to the same
  conversation and thread timestamp from the normalized chat event. The sample
  source maps `replyAs` values to separate Slack bot token env vars for Yui,
  Mika, and Rina, and Riela records `replyAs` as `yui`, `mika`, and `rina` for
