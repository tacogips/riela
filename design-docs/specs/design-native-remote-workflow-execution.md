...
Issue title: Finish NRE-03 native Riela remote workflow execution receiver.
...
contains a prior generator repair but was omitted from NRE-03 writePaths. Step 4
must review that diff and `scripts/surface-parity/generate-sdl.sh`, then add only
that generated Swift path to NRE-03's manifest write/change-tracking ownership
and record its generator/verification rule. The script is a reviewed generation
