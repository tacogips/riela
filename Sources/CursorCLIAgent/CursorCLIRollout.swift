...
public enum CursorCLISessionSource: String, Codable, Equatable, Sendable {
  case cli
  case vscode
  case exec
  case unknown
...
public enum CursorCLIMessageOrigin: String, Codable, Equatable, Sendable {
...
public struct CursorCLIMessageProvenance: Codable, Equatable, Sendable {
  public var role: String?
  public var origin: CursorCLIMessageOrigin
  public var displayDefault: Bool
  public var sourceTag: String?

  public init(role: String? = nil, origin: CursorCLIMessageOrigin, displayDefault: Bool, sourceTag: String? = nil) {
    self.role = role
    self.origin = origin
...
public struct CursorCLIRolloutLine: Equatable, Sendable {
  public var timestamp: String
  public var type: String
  public var payload: JSONValue
  public var provenance: CursorCLIMessageProvenance?

  public init(timestamp: String, type: String, payload: JSONValue, provenance: CursorCLIMessageProvenance? = nil) {
...
public struct CursorCLISessionMeta: Equatable, Sendable {
...
public struct CursorCLISessionGit: Equatable, Sendable {
...
public enum CursorCLISessionMessageCategory: String, Equatable, Sendable {
...
public struct CursorCLISessionMessage: Equatable, Sendable {
  public var timestamp: String
  public var category: CursorCLISessionMessageCategory
  public var role: String
  public var text: String?
  public var sourceType: String
  public var sourceTag: String?
  public var line: CursorCLIRolloutLine

  public init(
    timestamp: String,
    category: CursorCLISessionMessageCategory,
    role: String,
    text: String?,
    sourceType: String,
    sourceTag: String? = nil,
    line: CursorCLIRolloutLine
  ) {
    self.timestamp = timestamp
...
public struct CursorCLISessionMessageOptions: Equatable, Sendable {
...
public enum CursorCLIRolloutReader {
  public static func parseRolloutLine(_ line: String) -> CursorCLIRolloutLine? {
  ...
  public static func readRollout(path: String) throws -> [CursorCLIRolloutLine] {
    let content = try String(contentsOfFile: path, encoding: .utf8)
    return content.split(separator: "\n", omittingEmptySubsequences: false).compactMap { parseRolloutLine(String($0)) }
  }

  public static func parseSessionMeta(path: String) throws -> CursorCLISessionMeta? {
    let content = try String(contentsOfFile: path, encoding: .utf8)
  ...
  public static func getSessionMessages(path: String, options: CursorCLISessionMessageOptions = CursorCLISessionMessageOptions()) throws -> [CursorCLISessionMessage] {
  ...
  public static func parseSessionMeta(from line: CursorCLIRolloutLine) -> CursorCLISessionMeta? {
    ...
    let source = CursorCLISessionSource(rawValue: stringValue(meta["source"]) ?? "") ?? .unknown
...
public func parseRolloutLine(_ line: String) -> CursorCLIRolloutLine? {
  CursorCLIRolloutReader.parseRolloutLine(line)
}

public func readRollout(path: String) throws -> [CursorCLIRolloutLine] {
  try CursorCLIRolloutReader.readRollout(path: path)
}

public func parseSessionMeta(path: String) throws -> CursorCLISessionMeta? {
  try CursorCLIRolloutReader.parseSessionMeta(path: path)
}

public func extractFirstUserMessage(path: String) throws -> String? {
  try CursorCLIRolloutReader.extractFirstUserMessage(path: path)
}

public func getSessionMessages(path: String, options: CursorCLISessionMessageOptions = CursorCLISessionMessageOptions()) throws -> [CursorCLISessionMessage] {
  try CursorCLIRolloutReader.getSessionMessages(path: path, options: options)
}

private func normalizeRolloutLine(_ object: JSONObject) -> CursorCLIRolloutLine? {
  if let timestamp = stringValue(object["timestamp"]), let type = stringValue(object["type"]), let payload = object["payload"] {
    return CursorCLIRolloutLine(timestamp: timestamp, type: type, payload: payload)
  }
  ...
  case "item.completed":
    ...
    if itemType == "agent_message", let text = stringValue(item["text"]) {
      return CursorCLIRolloutLine(timestamp: timestamp, type: "event_msg", payload: .object(["type": .string("AgentMessage"), "message": .string(text)]))
    }
    return CursorCLIRolloutLine(timestamp: timestamp, type: "response_item", payload: .object(item))
  case "turn.started":
    return CursorCLIRolloutLine(timestamp: timestamp, type: "event_msg", payload: .object(compactObject(["type": .string("TurnStarted"), "turn_id": object["turn_id"]])))
  case "turn.completed":
    return CursorCLIRolloutLine(timestamp: timestamp, type: "event_msg", payload: .object(compactObject(["type": .string("TurnComplete"), "turn_id": object["turn_id"], "usage": object["usage"]])))
  case "error":
    return CursorCLIRolloutLine(timestamp: timestamp, type: "event_msg", payload: .object(compactObject(["type": .string("Error"), "message": object["message"]])))
  default:
    return nil
  }
}
...
private func deriveProvenance(_ line: CursorCLIRolloutLine) -> CursorCLIMessageProvenance? {
  switch line.type {
  case "session_meta", "turn_context", "compacted":
    return CursorCLIMessageProvenance(origin: .frameworkEvent, displayDefault: false, sourceTag: line.type)
  case "event_msg":
    guard let payload = objectValue(line.payload) else {
      return CursorCLIMessageProvenance(origin: .frameworkEvent, displayDefault: false, sourceTag: "event_msg_unknown")
    }
    switch stringValue(payload["type"]) {
    case "UserMessage":
      let message = stringValue(payload["message"]) ?? ""
      if let sourceTag = detectSourceTag(message) {
        return CursorCLIMessageProvenance(role: "user", origin: .systemInjected, displayDefault: false, sourceTag: sourceTag)
      }
      return CursorCLIMessageProvenance(role: "user", origin: .userInput, displayDefault: true)
    case "AgentMessage":
      return CursorCLIMessageProvenance(role: "assistant", origin: .userInput, displayDefault: true)
    case "ExecCommandBegin", "ExecCommandEnd":
      return CursorCLIMessageProvenance(origin: .toolGenerated, displayDefault: false, sourceTag: "local_shell")
    default:
      return CursorCLIMessageProvenance(origin: .frameworkEvent, displayDefault: false, sourceTag: stringValue(payload["type"]) ?? "event_msg")
    }
  case "response_item":
    guard let payload = objectValue(line.payload) else {
      return CursorCLIMessageProvenance(origin: .frameworkEvent, displayDefault: false, sourceTag: "response_item_unknown")
    }
    switch stringValue(payload["type"]) {
    case "message":
      return CursorCLIMessageProvenance(role: stringValue(payload["role"]), origin: .userInput, displayDefault: true)
    case "function_call":
      return CursorCLIMessageProvenance(role: "assistant", origin: .toolGenerated, displayDefault: false, sourceTag: stringValue(payload["name"]) ?? "function_call")
    case "function_call_output":
      return CursorCLIMessageProvenance(role: "user", origin: .toolGenerated, displayDefault: false, sourceTag: "function_call_output")
    case "local_shell_call":
      return CursorCLIMessageProvenance(origin: .toolGenerated, displayDefault: false, sourceTag: "local_shell")
    default:
      return CursorCLIMessageProvenance(origin: .frameworkEvent, displayDefault: false, sourceTag: stringValue(payload["type"]) ?? "response_item")
    }
  default:
    return nil
  }
...
private func toSessionMessage(_ line: CursorCLIRolloutLine) -> CursorCLISessionMessage? {
  guard let payload = objectValue(line.payload) else {
    return nil
  }
  if line.type == "event_msg" {
    switch stringValue(payload["type"]) {
    case "UserMessage":
      return CursorCLISessionMessage(timestamp: line.timestamp, category: .otherMessage, role: "user", text: stringValue(payload["message"]), sourceType: line.type, sourceTag: line.provenance?.sourceTag, line: line)
    case "AgentMessage":
