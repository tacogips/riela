...
public struct CodexModelUsageStats: Codable, Equatable, Sendable {
...
public struct CodexDailyActivity: Codable, Equatable, Sendable {
...
public struct CodexUsageStats: Codable, Equatable, Sendable {
  public var totalSessions: Int
  public var totalMessages: Int
  public var firstSessionDate: String?
  public var lastComputedDate: String?
  public var modelUsage: [String: CodexModelUsageStats]
  public var recentDailyActivity: [CodexDailyActivity]

  public init(totalSessions: Int, totalMessages: Int, firstSessionDate: String?, lastComputedDate: String?, modelUsage: [String: CodexModelUsageStats], recentDailyActivity: [CodexDailyActivity]) {
...
public enum CodexUsageStatsCollector {
  ...
  public static func getCodexUsageStats(options: CodexUsageStatsOptions = CodexUsageStatsOptions()) -> CodexUsageStats? {
    ...
    for rolloutFile in rolloutFiles {
      let lines = (try? CodexRolloutReader.readRollout(path: rolloutFile)) ?? []
    ...
    let recentDailyActivity = (0..<recentDays).map { offset in
      ...
      guard let activity = dailyActivity[key] else {
        return CodexDailyActivity(date: key)
      }
      return activity.export(date: key)
    }

    let result = CodexUsageStats(
  ...
  private static func cacheValue(key: String, now: Date, value: CodexUsageStats?) {
  ...
  fileprivate struct CacheEntry {
    var key: String
    var expiresAt: Date
    var value: CodexUsageStats?
  }
}

private final class UsageStatsCache: @unchecked Sendable {
  private let lock = NSLock()
  private var entry: CodexUsageStatsCollector.CacheEntry?

  func get(key: String, now: Date) -> CodexUsageStatsCollector.CacheEntry? {
    lock.lock()
    defer { lock.unlock() }
  ...
  func set(_ entry: CodexUsageStatsCollector.CacheEntry) {
    lock.lock()
    self.entry = entry
    lock.unlock()
  }
...
private struct MutableModelUsageStats {
  ...
  func export() -> CodexModelUsageStats {
    CodexModelUsageStats(inputTokens: inputTokens, outputTokens: outputTokens, cacheReadInputTokens: cacheReadInputTokens, cacheCreationInputTokens: cacheCreationInputTokens)
  }
}
...
private struct MutableDailyActivity {
  ...
  func export(date: String) -> CodexDailyActivity {
    CodexDailyActivity(
      date: date,
...
private func normalizeRecentDays(_ value: Int?) -> Int {
  guard let value, value > 0 else {
    return CodexUsageStatsCollector.defaultRecentDays
  }
  return value
}
...
private func isUserOrAssistantMessage(_ line: CodexRolloutLine) -> Bool {
...
private func extractToolCallCount(_ line: CodexRolloutLine) -> Int {
  guard let payload = objectValue(line.payload) else {
    return 0
  }
...
private func extractUsageEvent(_ line: CodexRolloutLine) -> UsageEvent? {
...
private func dateKey(from date: Date) -> String {
  DateFormatter.codexUsageDate.string(from: date)
}
...
private extension DateFormatter {
  static let codexUsageDate: DateFormatter = {
