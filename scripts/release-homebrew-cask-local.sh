...
release_tag="${1:-}"
tap_cask_file="${2:-$repo_root/../homebrew-tap/Casks/riela.rb}"
...
case "$release_tag" in
  v*) ;;
  *)
    printf 'error: release tag must start with v, for example v0.1.15\n' >&2
    exit 1
    ;;
esac
...
printf '  brew install --cask riela\n'
