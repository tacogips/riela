...
`riela/chat-reply-worker` add-on.
...
- The authored `reply-to-chat` step targets a reusable node-registry entry that
  uses `addon.name:
"riela/chat-reply-worker"` without a workflow-local node implementation
  file.
- When dispatched from the `example-reply-webhook` event source with
  `RIELA_EXAMPLE_REPLY_ENDPOINT` configured, the node emits one outbound chat
  reply request.
