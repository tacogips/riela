...
registry is `https://github.com/tacogips/riela-packages`, with a local
development checkout at `<repo-root>-packages`.
Personal registry configuration is stored under `~/.riela`, package
manifests are named `riela-package.json`, package changes are tracked with
...
`tacogips/riela-packages` repository. Those are owned by package command,
...
#### `packages/riela/src/workflow/packages/registry-config.ts`
...
- [x] Load missing config as effective default config under `~/.riela/workflow-packages/registries.json`.
...
**Checklist**:
- [x] Parse and normalize `riela-package.json` with `workflowDirectory` defaulting to `"."`.
...
#### `packages/riela/src/workflow/packages/cache.ts`
...
#### `packages/riela/src/workflow/packages/search.ts`
...
#### `packages/riela/src/workflow/packages/publish-metadata.ts`
...
|--------|-----------|--------|-------|
| Registry config | `packages/riela/src/workflow/packages/registry-config.ts` | COMPLETED | Focused package tests |
| Package manifest | `packages/riela/src/workflow/packages/manifest.ts` | COMPLETED | Focused package tests |
| Checksum service | `packages/riela/src/workflow/packages/checksum.ts` | COMPLETED | Focused package tests |
| Search index/cache | `packages/riela/src/workflow/packages/index.ts`, `packages/riela/src/workflow/packages/cache.ts` | IN_PROGRESS | Focused package tests |
| Search matching | `packages/riela/src/workflow/packages/search.ts` | COMPLETED | Focused package tests |
| Checkout/publish metadata | `packages/riela/src/workflow/packages/checkout.ts`, `packages/riela/src/workflow/packages/publish.ts` | IN_PROGRESS | Focused package tests |
| Public exports/docs | `packages/riela/src/workflow/packages/index.ts`, `README.md` | IN_PROGRESS | Typecheck and docs review |
...
**Status**: Completed
**Parallelizable**: Yes
**Deliverables**: `packages/riela/src/workflow/packages/registry-config.ts`, `packages/riela/src/workflow/packages/registry-config.test.ts`
**Dependencies**: None

**Completion Criteria**:
- [x] Default registry config resolves to `https://github.com/tacogips/riela-packages`.
- [x] Personal registry config persists under `~/.riela/workflow-packages/registries.json`.
...
**Status**: Completed
**Parallelizable**: Yes
**Deliverables**: `packages/riela/src/workflow/packages/manifest.ts`, `packages/riela/src/workflow/packages/manifest.test.ts`
**Dependencies**: None
...
**Status**: Completed
**Parallelizable**: No
**Deliverables**: `packages/riela/src/workflow/packages/checksum.ts`, `packages/riela/src/workflow/packages/checksum.test.ts`
...
**Status**: In Progress
**Parallelizable**: No
**Deliverables**: `packages/riela/src/workflow/packages/index.ts`, `packages/riela/src/workflow/packages/cache.ts`, related tests
...
**Status**: Completed
**Parallelizable**: No
**Deliverables**: `packages/riela/src/workflow/packages/search.ts`, `packages/riela/src/workflow/packages/search.test.ts`
...
**Status**: In Progress
**Parallelizable**: No
**Deliverables**: `packages/riela/src/workflow/checkout/registry.ts`, `packages/riela/src/workflow/packages/publish-metadata.ts`, related tests
...
**Status**: In Progress
**Parallelizable**: No
**Deliverables**: `packages/riela/src/workflow/packages/index.ts`, `packages/riela/src/lib.ts`, `README.md`
...
```bash
git diff --check -- impl-plans/active/workflow-package-registry.md packages/riela/src/workflow packages/riela/src/lib.ts README.md
bun test packages/riela/src/workflow/packages/registry-config.test.ts
bun test packages/riela/src/workflow/packages/manifest.test.ts
bun test packages/riela/src/workflow/packages/checksum.test.ts
bun test packages/riela/src/workflow/packages/cache.test.ts
bun test packages/riela/src/workflow/packages/search.test.ts
bun test packages/riela/src/workflow/checkout/checkout.test.ts
bun test packages/riela/src/workflow/catalog.test.ts
bun test packages/riela/src/workflow/usage.test.ts
bun test packages/riela/src/workflow/runtime-db.test.ts
bun run packages/riela/src/bin.ts workflow validate <checked-out-workflow-name>
bun run packages/riela/src/bin.ts workflow usage <checked-out-workflow-name> --output json
bunx tsc --noEmit
```

## Completion Criteria

- [x] Registry config persistence under `~/.riela/workflow-packages/registries.json` is implemented and tested.
- [x] `riela-package.json` parsing, normalization, safe `workflowDirectory` handling, and backend metadata derivation are implemented and tested.
...
- Moving current-dir `.riela` content into the default registry can disrupt local workflow usage if project catalog behavior is not preserved by the migration slice.
...
**Blockers**: Full repository Biome check is blocked by unrelated pre-existing formatting issues outside this feature slice and `packages/riela/src/workflow/validate/node-payload-validation.ts` exceeding 1000 lines.
**Notes**: Focused verification passed with `bun test packages/riela/src/workflow/packages/packages.test.ts`, `bun run typecheck`, and touched-file `bunx biome check --diagnostic-level=warn ...`. Step 6 self-review tightened authored backend validation and derived backend indexing from workflow node payloads.
