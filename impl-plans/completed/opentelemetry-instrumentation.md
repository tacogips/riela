...
Add coarse function-level OpenTelemetry tracing for riela workflow
...
`RIELA_OTEL_EXPORT_MESSAGES`. Add a repository-owned
...
- `<local-reference-path>`: inspected by Step 3; no relevant
  OpenTelemetry, OTEL, telemetry, trace, or Jaeger implementation found.
- `<local-reference-path>`: inspected by Step 3; no
...
#### packages/riela/src/telemetry/config.ts
#### packages/riela/src/telemetry/redaction.ts
#### packages/riela/src/telemetry/tracing.ts
#### packages/riela/src/telemetry/index.ts
#### packages/riela/src/telemetry/*.test.ts
#### package.json
...
- [x] Parse `OTEL_SDK_DISABLED`, `OTEL_SERVICE_NAME`,
      ...
      `OTEL_EXPORTER_OTLP_PROTOCOL`, `RIELA_OTEL_ENABLED`,
      `RIELA_OTEL_SERVICE_NAME`, `RIELA_OTEL_EXPORT_MESSAGES`,
...
#### packages/riela/src/lib-sessions.ts
#### packages/riela/src/cli.ts
#### packages/riela/src/server/serve.ts
#### packages/riela/src/events/listener-service.ts
#### packages/riela/src/events/http-routes.ts
#### packages/riela/src/index.ts
...
#### packages/riela/src/events/*.test.ts
...
#### packages/riela/src/workflow/engine.ts
#### packages/riela/src/workflow/engine/workflow-runner.ts
#### packages/riela/src/workflow/engine/node-execution.ts
#### packages/riela/src/workflow/engine/session-entry.ts
#### packages/riela/src/workflow/call-step-impl.ts
#### packages/riela/src/workflow/call-step-impl/*.ts
...
#### packages/riela/src/workflow/adapter-execution.ts
#### packages/riela/src/workflow/adapters/dispatch.ts
#### packages/riela/src/workflow/adapters/codex.ts
#### packages/riela/src/workflow/adapters/claude.ts
#### packages/riela/src/workflow/adapters/openai-sdk.ts
#### packages/riela/src/workflow/adapters/anthropic-sdk.ts
#### packages/riela/src/workflow/adapters/cursor.ts
#### packages/riela/src/workflow/adapters/*.test.ts
...
#### packages/riela/src/workflow/communication-service.ts
#### packages/riela/src/workflow/node-execution-mailbox.ts
#### packages/riela/src/workflow/engine/mailbox-and-communications.ts
#### packages/riela/src/workflow/engine/mailbox-communication-artifacts.ts
...
#### packages/riela/src/server/graphql.ts
#### packages/riela/src/server/graphql-executable-schema.ts
#### packages/riela/src/graphql/schema/execution-resolvers.ts
#### packages/riela/src/graphql/schema/supervisor-resolvers.ts
#### packages/riela/src/graphql/control-plane-service.ts
#### packages/riela-graphql/src/control-plane-service.ts
#### packages/riela-server/src/index.ts
#### packages/riela/src/graphql/schema.test.ts
#### packages/riela/src/server/*.test.ts
...
- [x] Include an example workflow run command with
      `OTEL_SERVICE_NAME=riela` and `OTEL_EXPORTER_OTLP_ENDPOINT`
      configured.
...
- [x] Verify traces reached Jaeger by querying the Jaeger API for the
      `riela` service and at least one returned trace before cleanup, or
      document a manual UI inspection when `curl`/`jq` is unavailable.
- [x] Document optional `RIELA_OTEL_EXPORT_MESSAGES=true` only for trusted
      fixtures.
...
|--------|-----------|--------|-------|
| Shared telemetry core | `packages/riela/src/telemetry/*`, `package.json` | Completed | Required |
| Entrypoint/library/event config | `packages/riela/src/lib*.ts`, `packages/riela/src/cli.ts`, `packages/riela/src/server/serve.ts`, `packages/riela/src/events/listener-service.ts`, `packages/riela/src/events/http-routes.ts` | Completed | Required |
| Workflow execution spans | `packages/riela/src/workflow/engine*`, `packages/riela/src/workflow/call-step-impl*` | Completed | Required |
| Backend adapter spans | `packages/riela/src/workflow/adapter-execution.ts`, `packages/riela/src/workflow/adapters/*` | Completed | Required |
| Communication/mailbox spans | `packages/riela/src/workflow/communication*.ts`, `packages/riela/src/workflow/node-execution-mailbox.ts` | Completed | Required |
| Server/GraphQL spans | `packages/riela/src/server/*`, `packages/riela/src/graphql/*`, `packages/riela-graphql/src/*` | Completed | Required |
...
**Status**: Completed
**Parallelizable**: No
**Deliverables**: `packages/riela/src/telemetry/*`, `package.json`,
...
- [x] Verification checks Jaeger for the `riela` service and at least one
...
- `bun run typecheck`
- `bun test packages/riela/src/telemetry/*.test.ts`
- `bun test packages/riela/src/workflow/engine.test.ts packages/riela/src/workflow/call-step-impl-execution.test.ts`
- `bun test packages/riela/src/workflow/communication-service.test.ts`
- `bun test packages/riela/src/workflow/adapters/*.test.ts`
- `bun test packages/riela/src/graphql/schema.test.ts packages/riela/src/server/serve.test.ts packages/riela/src/server/api.test.ts`
- `bun test packages/riela/src/events/listener-service.test.ts packages/riela/src/events/config.test.ts`
- `OTEL_SDK_DISABLED=true bun run packages/riela/src/bin.ts events serve --help`
- `bun test`
- `docker compose -f compose.jaeger.yaml up -d`
- `docker compose -f compose.jaeger.yaml ps`
- `OTEL_SERVICE_NAME=riela OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4318 bun run packages/riela/src/bin.ts workflow run first-four-arithmetic-pipeline --workflow-definition-dir ./examples --mock-scenario ./examples/first-four-arithmetic-pipeline/mock-scenario.json --output json`
- `curl -fsS http://localhost:16686/api/services | jq -e '.data | index("riela") != null'`
- `curl -fsS 'http://localhost:16686/api/traces?service=riela&limit=20' | jq -e '[.data[]?.spans[]?.operationName] | length > 0'`
...
- [x] `WorkflowTelemetryOptions.exportMessages` and
      `RIELA_OTEL_EXPORT_MESSAGES` control inbox/outbox payload export.
...
Also aligned explicit `RIELA_OTEL_ENABLED=true` with exporter defaults when
...
`packages/riela/src/workflow/adapter-execution.ts`, added an adapter test
