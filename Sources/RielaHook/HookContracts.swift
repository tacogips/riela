...
public struct HookRecordingControls: Codable, Equatable, Sendable {
  ...
  public init(env: [String: String]) throws {
    let recordingValue = env["RIELA_HOOK_RECORDING"]?.nilIfEmpty ?? "auto"
    let captureValue = env["RIELA_HOOK_CAPTURE_RAW"]?.nilIfEmpty ?? "redacted"
    guard let recording = HookRecordingMode(rawValue: recordingValue) else {
      throw HookContractError.invalidPayload("RIELA_HOOK_RECORDING must be 'auto', 'off', or 'required'")
    }
    guard let captureRaw = HookRawCaptureMode(rawValue: captureValue) else {
      throw HookContractError.invalidPayload("RIELA_HOOK_CAPTURE_RAW must be 'redacted', 'metadata-only', or 'full'")
    }
    self.recording = recording
    self.strict = ["1", "true", "yes"].contains((env["RIELA_HOOK_STRICT"] ?? "").lowercased())
    self.captureRaw = captureRaw
  }
}

public struct RielaHookContext: Codable, Equatable, Sendable {
...
public enum HookContextResolver {
  public static func resolveRielaHookContext(payload: JSONObject, env: [String: String], controls: HookRecordingControls? = nil) throws -> RielaHookContext? {
    let controls = try controls ?? HookRecordingControls(env: env)
    if controls.recording == .off {
      return nil
    }
    let workflowId = readEnvValue(env, key: "RIELA_WORKFLOW_ID")
    let workflowExecutionId = readEnvValue(env, key: "RIELA_WORKFLOW_EXECUTION_ID")
    let nodeId = readEnvValue(env, key: "RIELA_NODE_ID") ?? readEnvValue(env, key: "RIELA_MANAGER_STEP_ID")
    let nodeExecId = readEnvValue(env, key: "RIELA_NODE_EXEC_ID") ?? readEnvValue(env, key: "RIELA_MANAGER_NODE_EXEC_ID")
    guard let workflowId, let workflowExecutionId, let nodeId, let nodeExecId else {
      if controls.recording == .required {
        throw HookContractError.invalidPayload("missing riela hook context; required env vars are RIELA_WORKFLOW_ID, RIELA_WORKFLOW_EXECUTION_ID, RIELA_NODE_ID, and RIELA_NODE_EXEC_ID")
      }
      return nil
    }
    ...
    return .init(
      ...
      managerSessionId: readEnvValue(env, key: "RIELA_MANAGER_SESSION_ID"),
      agentBackend: readEnvValue(env, key: "RIELA_AGENT_BACKEND")
    )
  }
