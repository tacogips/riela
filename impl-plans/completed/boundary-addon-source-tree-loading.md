...
workflow validation resolves newly added built-in `riela/*` add-ons from the
source tree instead of accepting stale `packages/riela-addons/dist/index.js`
output.
...
Blocking finding:
`packages/riela/src/workflow/addon-package-boundary.ts:82` imports
`packages/riela-addons/dist/index.js` before source when riela runs from
`packages/riela/src`; stale dist output hides newly added built-in add-ons
such as `riela/workflow-package-sandbox-review`.
...
- Detect whether the riela entrypoint URL is under
  `packages/riela/src`.
- For source-tree execution, prefer
  `packages/riela-addons/src/index.ts` before
  `packages/riela-addons/dist/index.js`.
...
- [x] `createBoundaryAddonPackageLoader` imports source first when the
      riela entrypoint is under `packages/riela/src`.
...
- Add a unit test proving source-tree entrypoints prefer
  `packages/riela-addons/src/index.ts` even when a built entrypoint URL is
  present.
...
- [x] Built-in `riela/workflow-package-sandbox-review` validation succeeds
...
- [x] Add-on node validation result remains present with source `addon` and
      addon name `riela/workflow-package-sandbox-review`.
...
|--------|-----------|--------|-------|
| Boundary add-on package loader | `packages/riela/src/workflow/addon-package-boundary.ts` | COMPLETED | `addon-package-boundary.test.ts`, `validate.test.ts` |
| Boundary loader regression tests | `packages/riela/src/workflow/addon-package-boundary.test.ts` | COMPLETED | `bun test packages/riela/src/workflow/addon-package-boundary.test.ts` |
| Workflow validation regression | `packages/riela/src/workflow/validate.test.ts` | COMPLETED | `bun test packages/riela/src/workflow/validate.test.ts` |
...
**Status**: COMPLETED
**Parallelizable**: No
**Deliverables**: `packages/riela/src/workflow/addon-package-boundary.ts`
...
- [x] Source-tree `packages/riela/src` entrypoint resolves source before
      dist.
...
**Deliverables**: `packages/riela/src/workflow/addon-package-boundary.test.ts`
...
- [x] `bun test packages/riela/src/workflow/addon-package-boundary.test.ts`
      passes.
...
**Deliverables**: `packages/riela/src/workflow/validate.test.ts`

Keep the `riela/workflow-package-sandbox-review` validation tests aligned
...
- [x] `bun test packages/riela/src/workflow/validate.test.ts` passes.
...
- [x] `bun test packages/riela/src/workflow/addon-package-boundary.test.ts`
      passes.
- [x] `bun test packages/riela/src/cli.test.ts` passes or any failure is
...
```bash
bun run typecheck
bun test packages/riela/src/workflow/validate.test.ts
...
```bash
bun test packages/riela/src/workflow/packages/checkout.test.ts packages/riela/src/workflow/packages/packages.test.ts
bun test packages/riela/src/workflow/adapters/codex.test.ts packages/riela/src/workflow/adapters/cursor.test.ts
```
...
- [x] Newly added built-in add-ons exported from
      `packages/riela-addons/src/index.ts` are visible during source-tree
...
- PASS: `bun test packages/riela/src/workflow/addon-package-boundary.test.ts packages/riela/src/workflow/validate.test.ts packages/riela/src/cli.test.ts`

**Notes**: `packages/riela/src/workflow/addon-package-boundary.ts` now
derives an explicit import order from the riela entrypoint path, preferring
`packages/riela-addons/src/index.ts` for `packages/riela/src` execution
