...
public struct ClaudeCodeModelUsageStats: Codable, Equatable, Sendable {
...
public struct ClaudeCodeDailyActivity: Codable, Equatable, Sendable {
...
public struct ClaudeCodeUsageStats: Codable, Equatable, Sendable {
  public var totalSessions: Int
  public var totalMessages: Int
  public var firstSessionDate: String?
  public var lastComputedDate: String?
  public var modelUsage: [String: ClaudeCodeModelUsageStats]
  public var recentDailyActivity: [ClaudeCodeDailyActivity]

  public init(totalSessions: Int, totalMessages: Int, firstSessionDate: String?, lastComputedDate: String?, modelUsage: [String: ClaudeCodeModelUsageStats], recentDailyActivity: [ClaudeCodeDailyActivity]) {
...
public enum ClaudeCodeUsageStatsCollector {
  ...
  public static func getClaudeCodeUsageStats(options: ClaudeCodeUsageStatsOptions = ClaudeCodeUsageStatsOptions()) -> ClaudeCodeUsageStats? {
    ...
    for rolloutFile in rolloutFiles {
      let lines = (try? ClaudeCodeRolloutReader.readRollout(path: rolloutFile)) ?? []
    ...
    let recentDailyActivity = (0..<recentDays).map { offset in
      ...
      guard let activity = dailyActivity[key] else {
        return ClaudeCodeDailyActivity(date: key)
      }
      return activity.export(date: key)
    }

    let result = ClaudeCodeUsageStats(
  ...
  private static func cacheValue(key: String, now: Date, value: ClaudeCodeUsageStats?) {
  ...
  fileprivate struct CacheEntry {
    var key: String
    var expiresAt: Date
    var value: ClaudeCodeUsageStats?
  }
}

private final class UsageStatsCache: @unchecked Sendable {
  private let lock = NSLock()
  private var entry: ClaudeCodeUsageStatsCollector.CacheEntry?

  func get(key: String, now: Date) -> ClaudeCodeUsageStatsCollector.CacheEntry? {
    lock.lock()
    defer { lock.unlock() }
  ...
  func set(_ entry: ClaudeCodeUsageStatsCollector.CacheEntry) {
    lock.lock()
    self.entry = entry
    lock.unlock()
  }
...
private struct MutableModelUsageStats {
  ...
  func export() -> ClaudeCodeModelUsageStats {
    ClaudeCodeModelUsageStats(inputTokens: inputTokens, outputTokens: outputTokens, cacheReadInputTokens: cacheReadInputTokens, cacheCreationInputTokens: cacheCreationInputTokens)
  }
}
...
private struct MutableDailyActivity {
  ...
  func export(date: String) -> ClaudeCodeDailyActivity {
...
private func normalizeRecentDays(_ value: Int?) -> Int {
  guard let value, value > 0 else {
    return ClaudeCodeUsageStatsCollector.defaultRecentDays
  }
  return value
}
...
private func isUserOrAssistantMessage(_ line: ClaudeCodeRolloutLine) -> Bool {
...
private func extractToolCallCount(_ line: ClaudeCodeRolloutLine) -> Int {
  guard let payload = objectValue(line.payload) else {
    return 0
  }
...
private func extractUsageEvent(_ line: ClaudeCodeRolloutLine) -> UsageEvent? {
...
private func dateKey(from date: Date) -> String {
  DateFormatter.claudeCodeUsageDate.string(from: date)
}
...
private extension DateFormatter {
  static let claudeCodeUsageDate: DateFormatter = {
