...
`<repo-root>-packages/packages/greeting-node-addon`.
...
- Use the current uncommitted riela and riela-packages changes as the
...
- Codex-agent references:
  - `<repo-root>`: current uncommitted implementation
    baseline.
  - `<repo-root>-packages/packages/greeting-node-addon`:
    target executable node-addon package.
  - `<local-reference-path>`: behavioral process-command
    reference only; do not copy code or move Riela behavior into adapter
    boundaries.
...
#### `packages/riela-addons/src/local-node-addons.ts`
...
#### `packages/riela/src/workflow/packages/node-addon-install.ts`
#### `packages/riela/src/workflow/packages/checkout-node-addon.ts`
#### `packages/riela-addons/src/local-node-addons.ts`
...
- [x] Keep `riela/*` package-provided add-ons rejected.
...
#### `packages/riela-addons/src/local-node-addons.ts`
#### `packages/riela/src/workflow/addon-source-summary.ts`
#### `packages/riela/src/workflow/packages/install-validation.ts`
#### `packages/riela/src/workflow/load.ts`
#### `packages/riela/src/workflow/validate.ts`
#### `packages/riela/src/cli/argument-parser.ts`
#### `packages/riela/src/cli/workflow-run-command.ts`
...
#### `packages/riela-addons/src/native-node-executor/template-env-and-containers.ts`
...
#### `<repo-root>-packages/packages/greeting-node-addon/riela-package.json`
#### `<repo-root>-packages/packages/greeting-node-addon/addons/examples/greeting-shell/1/addon.json`
#### `<repo-root>-packages/packages/greeting-node-addon/addons/examples/greeting-shell/1/greeting.bash`
...
- [ ] Declare matching execution, capability, and digest metadata in
      `riela-package.json` and `addon.json`.
...
- [ ] Avoid unrelated riela-packages README or registry churn unless required
      for fixture correctness.

### 7. Focused Tests And Live Local Smoke

#### `packages/riela/src/workflow/packages/packages.test.ts`
#### `packages/riela/src/workflow/packages/*.test.ts`
#### `packages/riela-addons/src/**/*.test.ts`
...
| --- | --- | --- | --- |
| Executable manifest and dependency types | `packages/riela/src/workflow/packages/types.ts` | COMPLETED | Package tests |
| Local add-on manifest extension | `packages/riela-addons/src/local-node-addons.ts` | COMPLETED | Add-on tests |
| Manifest normalization and artifact validation | `packages/riela/src/workflow/packages/manifest.ts`, `node-addon-install.ts`, `checkout-node-addon.ts` | COMPLETED | Package tests |
| Dependency install and checkout provenance | `packages/riela/src/workflow/packages/dependencies.ts`, `checkout.ts`, `search.ts` | COMPLETED | Package tests |
| Installed executable resolution and direct grants | `packages/riela/src/workflow/load.ts`, `validate.ts`, `install-validation.ts`, `packages/riela/src/cli/argument-parser.ts`, `packages/riela/src/cli/workflow-run-command.ts` | COMPLETED | Workflow/package/CLI tests |
| Native command runtime path | `packages/riela-addons/src/native-node-executor/template-env-and-containers.ts` | COMPLETED | Add-on/runtime tests |
| Greeting node-addon package | `<repo-root>-packages/packages/greeting-node-addon` | COMPLETED | Live local smoke |
...
- `types-and-manifest-validation`: package manager types and manifest
  normalization in `packages/riela/src/workflow/packages/*`.
- `local-addon-runtime-resolution`: local add-on resolver and native executor
  files under `packages/riela-addons/src/*`.
...
bun run packages/riela/src/bin.ts package registry add local <repo-root>-packages --project-root "$tmp_project"
bun run packages/riela/src/bin.ts package install greeting-node-addon --project-root "$tmp_project" --output json
bun run packages/riela/src/bin.ts workflow run --workflow-json-file <temp-greeting-workflow.json> --project-root "$tmp_project" --direct-executable-addon-grant @"$grant_file" --output json
```
...
baseline and avoid unrelated `.riela` managed/provenance or
...
`packages/riela/src/workflow/packages/package-addon-locks.ts` so
...
`packages/riela-addons/src/local-node-addon-executable-validation.ts` to keep
...
`packages/riela-addons/src/local-node-addon-authorization.ts` to keep source
