...
or PR creation, migration of current `.riela` content into the default
...
- `packages/riela/src/cli/argument-parser.ts`
...
- `packages/riela/src/workflow/checkout/`
- `packages/riela/src/workflow/packages/cache.ts`
- `packages/riela/src/workflow/packages/checkout.ts`
- `packages/riela/src/workflow/packages/pre-install-check.ts`
- `packages/riela/src/workflow/packages/pre-install-scanner.ts`
- `packages/riela/src/workflow/packages/pre-install-container.ts`
...
- `packages/riela/src/workflow/validate/`
...
- [x] Write checkout record package fields under
      `~/.riela/workflow-registry/checkouts/<scope>-<workflow-name>.json`.
...
- [x] Accept `riela cli workflow checkout <package-id>` when the target is
      not a GitHub directory URL.
- [x] Keep `riela cli workflow checkout <github-url>` compatible.
- [x] Add `riela cli workflow search [query]` with `--registry`, `--tag`,
...
|--------|-----------|--------|-------|
| Package public types | `packages/riela/src/workflow/packages/types.ts` | COMPLETED | `packages/riela/src/workflow/packages/packages.test.ts` |
| Cache key safety | `packages/riela/src/workflow/packages/cache.ts` | COMPLETED | `packages/riela/src/workflow/packages/packages.test.ts` |
| Package search service | `packages/riela/src/workflow/packages/search.ts` | COMPLETED | `packages/riela/src/workflow/packages/packages.test.ts` |
| Package checkout service | `packages/riela/src/workflow/packages/checkout.ts` | IN_PROGRESS | `packages/riela/src/workflow/packages/packages.test.ts` |
| Direct checkout compatibility | `packages/riela/src/workflow/checkout/` | COMPLETED | existing checkout and CLI coverage |
| CLI command wiring | `packages/riela/src/cli/argument-parser.ts`, `packages/riela/src/cli/input-output-helpers.ts`, `packages/riela/src/cli/workflow-command-handler.ts`, `packages/riela/src/cli/workflow-package-command-handler.ts` | IN_PROGRESS | `packages/riela/src/cli.test.ts` |
| Pre-install check contract | `packages/riela/src/workflow/packages/types.ts` | READY | `packages/riela/src/workflow/packages/packages.test.ts` |
| Static scanner | `packages/riela/src/workflow/packages/pre-install-scanner.ts` | READY | `packages/riela/src/workflow/packages/packages.test.ts` |
| Container check | `packages/riela/src/workflow/packages/pre-install-container.ts` | READY | `packages/riela/src/workflow/packages/packages.test.ts` |
| Checkout ordering integration | `packages/riela/src/workflow/packages/checkout.ts`, `packages/riela/src/workflow/packages/integrity.ts` | READY | `packages/riela/src/workflow/packages/packages.test.ts` |
| Security CLI/docs | `packages/riela/src/cli/*`, `README.md` | READY | `packages/riela/src/cli.test.ts` |
...
bun test packages/riela/src/workflow/packages
bun test packages/riela/src/workflow/checkout/checkout.test.ts
bun test packages/riela/src/cli.test.ts
bun run packages/riela/src/bin.ts workflow package checkout <package-id> --help
bun run packages/riela/src/bin.ts workflow package checkout <package-id> --pre-install-check
bun run tsc --noEmit
git diff --check
```
...
```bash
riela cli workflow package registry add local --registry-url https://github.com/example/riela-packages --local-path /tmp/riela-packages --output json
riela cli workflow package search example --registry local --refresh --output json
riela cli workflow package checkout example-flow --registry local --overwrite --output json
riela cli workflow package checkout example-flow --registry local --overwrite --pre-install-check --output json
riela cli workflow package checkout example-flow --registry local --overwrite --pre-install-check --pre-install-check-mode warn --output json
riela cli workflow search example --registry local --refresh --output json
riela cli workflow checkout example-flow --registry local --overwrite --output json
```
...
**Notes**: Focused verification passed with `bun test packages/riela/src/workflow/packages/packages.test.ts`, `bun run typecheck`, and touched-file Biome checks.
...
**Notes**: `bun test packages/riela/src/cli.test.ts`, package tests, typecheck, touched-file Biome, and diff whitespace checks passed.
