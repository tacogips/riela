...
final class WorkflowCommandTests: XCTestCase {
  ...
  func testRemovedKaibaMemoryAddonsFailClosed() async {
    ...
    for addonName in removedAddonNames {
      do {
        _ = try await resolver.execute(
          WorkflowAddonExecutionInput(
            workflowId: "removed-memory-addon-tests",
            stepId: "removed-addon",
            nodeId: "removed-addon",
            addon: WorkflowNodeAddonRef(name: addonName, version: "1", config: [:]),
            variables: [:]
          ),
          context: AdapterExecutionContext()
        )
        XCTFail("Expected \(addonName) to be rejected")
      } catch let error as AdapterExecutionError {
        XCTAssertEqual(error.code, .providerError)
        XCTAssertTrue(error.message.contains("missing add-on resolver"), error.message)
      } catch {
