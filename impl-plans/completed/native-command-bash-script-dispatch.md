...
- The re-exported runtime path in
  `packages/riela/src/workflow/native-node-executor/template-env-and-containers.ts`.
...
- Relaxing direct execution requirements for non-`.sh` and non-`.bash`
  script paths.
- Refreshing `riela-package.json` digests unless workflow package scripts,
...
- Issue: `#46`
- Issue URL: `https://github.com/tacogips/riela/issues/46`
...
```typescript
export * from "../../../../riela-addons/src/native-node-executor/template-env-and-containers";
```

**Checklist**:

- [x] Confirm no duplicate dispatch implementation exists under `packages/riela/src`.
...
| --- | --- | --- | --- |
| Native command dispatch | `packages/riela-addons/src/native-node-executor/template-env-and-containers.ts` | COMPLETED | `bun test packages/riela/src/workflow/native-node-executor-gateway.test.ts` passed |
| Native command re-export boundary | `packages/riela/src/workflow/native-node-executor/template-env-and-containers.ts` | COMPLETED | Covered by gateway test |
| Regression coverage | `packages/riela/src/workflow/native-node-executor-gateway.test.ts` | COMPLETED | `bun test packages/riela/src/workflow/native-node-executor-gateway.test.ts` passed |
| Package scanner boundary | `packages/riela/src/workflow/packages/pre-install-scanner.ts` | COMPLETED | Inspection confirmed unchanged scanner warning boundary |
...
- Inspect
  `packages/riela/src/workflow/packages/pre-install-scanner.ts` only for
...
| TASK-002 | TASK-001 | `packages/riela-addons/src/native-node-executor/template-env-and-containers.ts` | COMPLETED |
| TASK-003 | TASK-002 | `packages/riela/src/workflow/native-node-executor-gateway.test.ts` | COMPLETED |
...
```bash
bun test packages/riela/src/workflow/native-node-executor-gateway.test.ts
bun run typecheck
bun run build:server
git diff --check -- packages/riela-addons/src/native-node-executor/template-env-and-containers.ts packages/riela/src/workflow/native-node-executor-gateway.test.ts impl-plans/active/native-command-bash-script-dispatch.md
```

Package-managed workflow smoke when available:

```bash
workflow run codex-source-security-check-loop --working-dir <repo-root>
```
...
- [x] `riela-package.json` digests are refreshed only if workflow package,
...
`packages/riela/src` executor file remains a re-export. The package
...
script, or skill files were edited, so no `riela-package.json` digest refresh
was required.
