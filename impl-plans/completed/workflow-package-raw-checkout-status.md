...
workflow checkout records created by `riela workflow checkout` from
registry-managed package checkout records created by `riela package install`.
...
- Issue reference: `tacogips/riela#47`
- Issue URL: `https://github.com/tacogips/riela/issues/47`
...
riela CLI/package-boundary issue.
...
| --- | --- | --- | --- |
| Raw checkout catalog projection | `packages/riela/src/workflow/packages/checkout-records.ts`, `packages/riela/src/workflow/packages/types.ts` | COMPLETED | `packages.test.ts` |
| Package list output | `packages/riela/src/workflow/packages/checkout-records.ts`, `packages/riela/src/cli/workflow-package-command-handler.ts` | COMPLETED | `packages.test.ts`, `cli.test.ts` |
| Package status fallback | `packages/riela/src/workflow/packages/checkout-records.ts`, `packages/riela/src/workflow/packages/status.ts` | COMPLETED | `packages.test.ts`, `cli.test.ts` |
| CLI text/help/docs | `packages/riela/src/cli/workflow-package-command-handler.ts`, `packages/riela/src/cli/input-output-helpers.ts`, `README.md` | COMPLETED | CLI status tests |
| Regression verification | `packages/riela/src/workflow/packages/packages.test.ts`, `packages/riela/src/cli.test.ts` | COMPLETED | Focused test commands |
...
- `bun test packages/riela/src/workflow/packages/packages.test.ts -t "package status"`
- `bun test packages/riela/src/workflow/packages/packages.test.ts -t "package list"`
- `bun test packages/riela/src/cli.test.ts -t "package status"`
- `bun test packages/riela/src/cli.test.ts -t "package list"`
- `USER_ROOT=$(mktemp -d); bun run packages/riela/src/bin.ts workflow checkout https://github.com/tacogips/riela-packages/tree/main/packages/codex-deepdesign/workflows/codex-deepdesign --user-scope --user-root "$USER_ROOT" --yes; bun run packages/riela/src/bin.ts workflow list --scope user --user-root "$USER_ROOT" --output json; bun run packages/riela/src/bin.ts workflow usage codex-deepdesign --scope user --user-root "$USER_ROOT"; bun run packages/riela/src/bin.ts package list --scope user --user-root "$USER_ROOT" --output json; bun run packages/riela/src/bin.ts package status codex-deepdesign --scope user --user-root "$USER_ROOT" --output json`
- `bunx tsc --noEmit`
- `bunx biome check .`
- `git diff --check`
