...
- `superviserWorkflowId` defaults to `riela-default-superviser` (see `examples/default-superviser/`). **Without** `--nested-superviser`, remediation is still the engine `runAutoImprove` loop. **With** `--nested-superviser`, that bundle runs as a nested step-addressed workflow and drives the target via `riela/*` control add-ons (the engine injects `supervisionRunId`, `targetSessionId`, and `superviserTargetWorkflowId` as runtime variables on the superviser session).
...
```bash
riela workflow run <workflow> --auto-improve \
