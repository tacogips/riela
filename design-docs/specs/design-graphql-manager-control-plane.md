...
The current `riela` runtime already has strong domain primitives:
...
- a first-class workflow manager interaction surface that `riela` LLM nodes can call directly,
...
5. The current `riela` executable is a runtime entrypoint, not a manager-tool client with ambient execution identity.
...
- long-term, the CLI becomes a thin GraphQL client; `riela graphql` is the canonical generic GraphQL command and legacy `gql` compatibility is not retained.
...
- Allow an `riela` manager node to call `riela graphql "<graphql document>"` from inside its LLM/tool environment.
...
GraphQL becomes the canonical control-plane endpoint exposed by `riela serve`:
...
- `riela graphql` supports GraphQL variables through a single `--variables` option that accepts inline JSON or a file reference syntax such as `@path/to/variables.json`
...
- session/source metadata under `packages/riela/src/workflow/session.ts`,
  `packages/riela/src/workflow/session-store.ts`,
  `packages/riela/src/workflow/runtime-db/session-query-records.ts`, and
...
- temporary-run provenance paths in
  `packages/riela/src/workflow/packages/temp-run.ts` and
  `packages/riela/src/cli/registry-run-provenance.ts`
...
- Podman or other container runtimes should be able to mount the same Riela data directory at a different host path while preserving the logical file reference
...
```text
{rielaRootDataDir}/files/{workflowId}/{workflowExecutionId}/attachments/{fileName}
```
...
- the runtime resolves the absolute filesystem path from the configured Riela root data directory
...
- attachment creation/upload is out of scope for the first iteration; files must already exist under the Riela root data directory before the GraphQL request is sent

### Configuration

The Riela root data directory is resolved from environment variable or config.

Design direction:

- introduce `RIELA_ARTIFACT_DIR` as the canonical root-data setting for derived defaults
...
3. derived path from `RIELA_ARTIFACT_DIR`
4. built-in default

Initial derived defaults:

- artifact root: `{RIELA_ARTIFACT_DIR}/workflow`
- session store root: `{RIELA_ARTIFACT_DIR}/sessions`
- attachments root: `{RIELA_ARTIFACT_DIR}/files`
...
- manager send is a control-plane request from an `riela` manager tool session to the orchestration runtime
...
- `RIELA_GRAPHQL_ENDPOINT`
- `RIELA_MANAGER_AUTH_TOKEN`
- `RIELA_MANAGER_SESSION_ID`
- `RIELA_WORKFLOW_ID`
- `RIELA_WORKFLOW_EXECUTION_ID`
- `RIELA_MANAGER_STEP_ID`
- `RIELA_MANAGER_NODE_EXEC_ID`

The explicit command form requested by the user is supported:

```bash
riela graphql "<graphql document>"
```
...
- for HTTP transport, `riela graphql` forwards `RIELA_MANAGER_SESSION_ID` in `X-Riela-Manager-Session-Id` so the server can resolve the scoped manager session without embedding it in GraphQL variables
...
- normal GraphQL HTTP transport forwards `managerSessionId` in `X-Riela-Manager-Session-Id`
- `RIELA_MANAGER_AUTH_TOKEN` is the CLI/env injection path used inside manager tool environments, not a separate authentication mechanism
...
- `managerSessionId` is forwarded in `X-Riela-Manager-Session-Id`
- GraphQL variables keep workflow-domain inputs only
- the `/graphql` HTTP handler must ignore any server-local `RIELA_MANAGER_*` or `RIELA_WORKFLOW_*` ambient execution variables for request authentication and scope resolution; only request transport metadata may supply manager scope on the HTTP boundary
- the `/graphql` HTTP handler must also ignore caller-provided in-process auth/session fallback fields when authenticating an HTTP request; `Authorization` and `X-Riela-Manager-Session-Id` remain the only manager-scope carriers on that boundary
...
- the runtime resolves them against `RIELA_ARTIFACT_DIR`
...
2. explicit `sendManagerMessage` via `riela graphql` control-plane command
...
`riela serve` continues to host the local control plane.
...
- allow the generic `riela graphql` CLI client to target the same local endpoint when remote transport is requested
...
5. Add the generic `riela graphql` CLI client.
...
- support LLM-triggered `riela graphql` through ambient manager-session context.
