...
This workflow demonstrates the riela-owned `discord-gateway` event source.
...
- `riela/chat-reply-worker` sends replies through
...
riela events emit discord-gateway-personas --workflow-definition-dir ./examples --event-root ./examples/event-sources/.riela-events --event-file ./examples/event-sources/payloads/discord-gateway-message-with-history.json --read-only --output json
