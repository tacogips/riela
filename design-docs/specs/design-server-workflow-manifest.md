...
This document defines the server workflow manifest used by `riela serve` to
publish an explicit workflow catalog for one server process.

## Overview

`riela serve` can run with a workflow manifest JSON file. When a manifest is
...
- `relative` must be a relative path and resolves from the current directory by
  default. `RIELA_WORKFLOW_MANIFEST_ROOT` overrides that base directory for
...
`riela serve` accepts `--workflow-manifest <path>`. The environment fallback is
`RIELA_WORKFLOW_MANIFEST`. Relative path fields inside the manifest resolve
from the current directory unless `RIELA_WORKFLOW_MANIFEST_ROOT` is set.

Startup resolution order for server workflow catalog selection:

1. `--workflow-manifest`
2. `RIELA_WORKFLOW_MANIFEST`
3. existing `serve [workflow-name]` plus `--workflow-definition-dir` /
   `RIELA_WORKFLOW_DEFINITION_DIR`
...
`--workflow-manifest`, or through `RIELA_WORKFLOW_MANIFEST`. The validation
...
- `RIELA_WORKFLOW_MANIFEST_ROOT` overriding the relative path root
