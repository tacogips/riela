...
support in the Riela workflow loader.
...
- Target feature area:
  `packages/riela/src/workflow/load.ts workflow definition inheritance`
...
- Inheritance helper:
  `packages/riela/src/workflow/load-inheritance.ts`
- Current branch tests:
  `packages/riela/src/workflow/load.test.ts`
- Behavioral package reference:
  `<repo-root>-packages/packages/cursor-cli-design-and-implement-review-loop/workflows/cursor-cli-design-and-implement-review-loop/workflow.json`
- Behavioral package reference:
  `<repo-root>-packages/packages/claude-code-design-and-implement-review-loop/workflows/claude-code-design-and-implement-review-loop/workflow.json`
...
- [x] Run `bun test packages/riela/src/workflow/load.test.ts`.
...
**Status**: Completed
**Parallelizable**: No
**Deliverables**: `packages/riela/src/workflow/load.ts`,
`packages/riela/src/workflow/load-inheritance.ts`
...
**Status**: Completed
**Parallelizable**: Yes
**Deliverables**: `packages/riela/src/workflow/load.test.ts`
**Dependencies**: None
...
- [x] `bun test packages/riela/src/workflow/load.test.ts` passes or failure
...
| ------ | --------- | ------ | ----- |
| Extends schema parsing and resolution guardrails | `packages/riela/src/workflow/load.ts`, `packages/riela/src/workflow/load-inheritance.ts` | COMPLETED | `packages/riela/src/workflow/load.test.ts` |
| In-memory derived bundle materialization | `packages/riela/src/workflow/load-inheritance.ts` | COMPLETED | `packages/riela/src/workflow/load.test.ts` |
| Focused loader regression tests | `packages/riela/src/workflow/load.test.ts` | COMPLETED | `bun test packages/riela/src/workflow/load.test.ts` |
| Verification and handoff | `packages/riela/src/workflow/load.ts`, `packages/riela/src/workflow/load-inheritance.ts`, `packages/riela/src/workflow/load.test.ts` | COMPLETED | `bun run typecheck` |
...
- `TASK-001` and `TASK-004` may proceed in parallel because `TASK-001` writes no
  code and `TASK-004` is limited to `packages/riela/src/workflow/load.test.ts`.
...
- `git diff -- design-docs/specs/design-workflow-json.md packages/riela/src/workflow/load.ts packages/riela/src/workflow/load-inheritance.ts packages/riela/src/workflow/load.test.ts`
- `bun test packages/riela/src/workflow/load.test.ts`
- `bun run typecheck`
...
`packages/riela/src/workflow/load.ts` and
`packages/riela/src/workflow/load-inheritance.ts`; added focused regression
coverage in `packages/riela/src/workflow/load.test.ts`; verified
`bun test packages/riela/src/workflow/load.test.ts` and
...
`bun test packages/riela/src/workflow/load.test.ts` passes with 25 tests and
