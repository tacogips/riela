...
final class RielaNoteUIAsyncS3Tests: XCTestCase {
  func testNoteServiceClientResolvesS3SourcePageImageThroughAsyncHTTPClient() async throws {
    ...
    let detail = try await client.noteDetail(noteId: note.noteId)
    let sourcePageImage = try XCTUnwrap(detail.files.first { $0.role == .sourcePageImage })
    let resolved = try await client.resolveFile(fileId: sourcePageImage.file.fileId)

    XCTAssertEqual(sourcePageImage.file.storageKind, .s3)
    XCTAssertEqual(resolved.data, imageData)
