...
accepted riela GraphQL/session design instead of copying agent behavior.
...
#### packages/riela-graphql/src/dto.ts
...
#### packages/riela/src/workflow/session.ts
#### packages/riela/src/workflow/session-store.ts
#### packages/riela/src/workflow/runtime-db/session-query-records.ts
...
#### packages/riela/src/workflow/packages/temp-run.ts
...
| ---- | ------ | ------------ | ------------ | -------------- |
| TASK-001 Public GraphQL contract | COMPLETED | `packages/riela-graphql/src/schema-contract.ts`, `packages/riela-graphql/src/dto.ts`, `packages/riela/src/graphql/types.ts` | Accepted design | Yes, with TASK-002 only |
| TASK-002 Snapshot persistence | COMPLETED | `packages/riela/src/workflow/session.ts`, `packages/riela/src/workflow/load.ts`, `packages/riela/src/workflow/engine/session-entry.ts` | Accepted design | Yes, with TASK-001 only |
| TASK-003 Temporary source handling | COMPLETED | Shared loaded-workflow snapshot path before cleanup | TASK-002 | No |
| TASK-004 Resolver wiring | COMPLETED | `packages/riela/src/graphql/schema/execution-resolvers.ts` | TASK-001, TASK-002 | No |
...
bun test packages/riela/src/graphql/schema.test.ts
bun test packages/riela/src/server/graphql-execution-overview-and-definitions.test.ts
bun test packages/riela/src/server/graphql-queries-and-inspection.test.ts
git diff --check
```
...
- `bun test packages/riela/src/workflow/load.test.ts packages/riela/src/graphql/schema.test.ts`
- `bun run typecheck`
- `bun run lint:biome`
- `git diff --check`
