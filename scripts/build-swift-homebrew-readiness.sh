...
Environment:
  ...
  RIELA_SWIFT_RELEASE_DIR   Output directory. Defaults to dist/swift-homebrew.
...
swift_release_bin_path() {
  ...
  (
    ...
    DEVELOPER_DIR="$developer_dir" SDKROOT="$sdkroot" \
      "$swift_bin" build -c release --product riela >/dev/null
    DEVELOPER_DIR="$developer_dir" SDKROOT="$sdkroot" \
      "$swift_bin" build -c release --product riela --show-bin-path
  )
}
...
print_plan() {
  ...
  archive="$release_dir/riela-swift-$version-$target.tar.gz"
  ...
  printf '  target: %s\n' "$target"
  printf '  release bin path command: swift build -c release --product riela --show-bin-path\n'
...
main() {
  ...
  release_dir="$(absolute_path "${RIELA_SWIFT_RELEASE_DIR:-dist/swift-homebrew}")"
