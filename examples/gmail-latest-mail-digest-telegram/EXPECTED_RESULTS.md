...
- `read-mail-state` uses `stateBackend: "kv"` to read seen message ids from the workflow key-value store (store `gmail-digest`, key `digest-state`, scope = this workflow id, database `<kvRoot>/gmail-digest.sqlite`; `kvRoot` defaults to `.riela/kv` under the working directory) and emits a max-10 Gmail fetch request.
- `fetch-latest-gmail` runs `riela/gmail-gateway-read` in Docker with `ghcr.io/tacogips/gmail-gateway:latest`, invokes the read-only `gmail-gateway-reader` surface through `threads(input:)`, and maps Gmail gateway credentials from `GMAIL_GATEWAY_CONFIG` only.
- `fetch-latest-gmail` requests live message metadata plus `textBody`/`htmlBody` and attachment file-reference metadata such as `downloadKey` when the gateway exposes it. It does not request raw attachment file payloads in GraphQL.
- `normalize-new-mail` treats the gateway result as untrusted data, accepts the current `threads.edges.node.messages` shape plus message file metadata payloads, keeps only fetched Gmail messages whose ids are not in the persisted state, and lets a first run notify about the latest 10 messages. When body payloads are present, it writes them under `.riela-data/gmail-latest-mail-digest-telegram/messages/` and passes only file metadata downstream.
...
- `persist-mail-state` uses `stateBackend: "kv"` to upsert fetched message ids into the same key-value entry before Telegram delivery and only routes to Telegram when `when.should_send_telegram` is true.
- The persisted state stores message ids and metadata only. Raw fetched email content can appear in workflow artifacts, so live runs should use an ignored artifact root such as `.riela-artifact/gmail-latest-mail-digest-telegram`.
