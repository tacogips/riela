...
This document defines the riela event-source design for Vercel Chat SDK
platform adapters.

## Overview

riela currently has event-source coverage for cron, generic webhook, Matrix,
...
package. riela should be able to receive a normalized webhook from an
...
- Normalize inbound provider events into riela `chat.message` events.
...
```json
{
  ...
  "webhook": {
    ...
    "signingSecretEnv": "RIELA_CHAT_SDK_SLACK_WEBHOOK_SECRET"
  },
  "send": {
    "endpointUrlEnv": "RIELA_CHAT_SDK_SLACK_SEND_URL",
    "tokenEnv": "RIELA_CHAT_SDK_SLACK_SEND_TOKEN"
  }
}
```
...
The generic boundary lets riela integrate with a deployed Chat SDK bot without
depending on the Chat SDK package graph in the riela runtime.
...
must store content under the riela data root before passing only refs to
...
- it does not introduce direct `@chat-adapter/*` dependencies in riela runtime
...
```bash
bun run packages/riela/src/main.ts events validate --workflow-definition-dir ./examples --event-root ./examples/event-sources/.riela-events
bun run packages/riela/src/main.ts workflow validate chat-event-attachment-judgement --workflow-root ./examples
bun run packages/riela/src/main.ts workflow inspect chat-event-attachment-judgement --workflow-root ./examples --output json
bun test packages/riela/src/events/adapters/chat-sdk.test.ts packages/riela/src/events/input-mapping.test.ts packages/riela/src/events/chat-reply-example.test.ts
```

Verification commands for the implementation plan:

```bash
rg -n "chat-sdk|chat\\.message|attachments|attachment judgement|outputDestinations" design-docs examples packages/riela/src README.md
bun run typecheck
bun test packages/riela/src/events/adapter-registry.test.ts packages/riela/src/events/config.test.ts packages/riela/src/events/listener-service.test.ts packages/riela/src/events/manual-emit.test.ts packages/riela/src/events/reply-dispatcher.test.ts
```
...
These are adapter behavior references only. riela intentionally keeps Chat
