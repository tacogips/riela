...
public struct SpecialistSupervisorStore: Sendable {
  ...
  func dispatch(taskId: String, in database: SQLiteDatabase) throws -> String? {
    guard let json = try database.query(
  ...
  func saveDeliveryReceipt(_ receipt: SpecialistDeliveryReceipt, in database: SQLiteDatabase) throws {
    try database.execute(
