...
- `read-digest-state` uses the built-in `riela/x-digest` add-on with `stateBackend: "kv"`, reading the previous cursor from the workflow key-value store (store `x-digest`, key `digest-state`, scope = this workflow id, database `<kvRoot>/x-digest.sqlite`) and emitting the previous `lastPostId` as the next `sinceId`. `kvRoot` resolves from add-on config, then workflow input, then `.riela/kv` under the working directory; mock runs can isolate the store with `--variables '{"workflowInput":{"kvRoot":".riela-data/x-follower-ai-business-digest/mock-kv"}}'`.
...
- `persist-digest-state` uses `riela/x-digest` with `stateBackend: "kv"` to upsert the newest fetched post id into the same key-value entry before Telegram delivery and only routes to Telegram when `when.should_send_telegram` is true.
- Only the newest post id plus metadata are persisted (with the file backend, `riela/x-digest` additionally rejects cursor state paths outside ignored/private runtime directories). Raw fetched posts may appear in workflow artifacts, so live runs should use an ignored artifact root such as `.riela-artifact/x-follower-ai-business-digest`.
