...
`riela package install` must install declared package dependencies before
...
- Issue: `https://github.com/tacogips/riela/issues/43`
...
- `codex-agent`: backend value inside package workflows only; no special install
  path.
- `<repo-root>`: local implementation repository.
- `<local-reference-path>`: preferred local behavioral reference
  only; do not copy code.
- `<repo-root>-packages/packages/codex-impl-plan-completion-loop/riela-package.json`
- `<repo-root>-packages/packages/codex-recent-change-quality-loop/riela-package.json`
- `<repo-root>-packages/packages/codex-refactoring-divide-and-conquer/riela-package.json`
...
- `packages/riela/src/workflow/packages/checkout-records.ts`
...
#### `packages/riela/src/workflow/packages/dependencies.ts`
#### `packages/riela/src/workflow/packages/search.ts`
...
#### `packages/riela/src/workflow/packages/checkout.ts`
#### `packages/riela/src/workflow/packages/skill-install.ts`
...
| --- | --- | --- | --- |
| Manifest dependency types | `packages/riela/src/workflow/packages/types.ts`, `packages/riela/src/workflow/packages/manifest.ts` | COMPLETED | Manifest normalization tests |
| Dependency resolver | `packages/riela/src/workflow/packages/dependencies.ts`, `packages/riela/src/workflow/packages/search.ts` | COMPLETED | Resolver graph and cycle tests |
| Satisfaction checks | `packages/riela/src/workflow/packages/dependencies.ts`, `packages/riela/src/workflow/packages/checkout.ts` | COMPLETED | Satisfied dependency tests |
| Install transaction | `packages/riela/src/workflow/packages/checkout.ts`, `packages/riela/src/workflow/packages/skill-install.ts` | COMPLETED | Install, rollback, validation visibility tests |
| CLI JSON output and docs | `packages/riela/src/cli/workflow-package-command-handler.ts`, `README.md` | COMPLETED | CLI issue reproduction output |
| Regression fixtures | `packages/riela/src/workflow/packages/packages.test.ts`, `packages/riela/src/cli.test.ts` | COMPLETED | Focused package install regressions and targeted package CLI test pass; full CLI suite still has one unrelated temporary resume failure |
...
- `packages/riela/src/workflow/packages/types.ts`
- `packages/riela/src/workflow/packages/manifest.ts`
- Manifest-focused tests in `packages/riela/src/workflow/packages/packages.test.ts`
...
- `packages/riela/src/workflow/packages/checkout-records.ts`
- `packages/riela/src/workflow/packages/checkout.ts`
- Optional helper changes in `packages/riela/src/workflow/packages/skill-install.ts`
...
- [x] Focused package checkout tests pass with
      `bun test packages/riela/src/workflow/packages/packages.test.ts packages/riela/src/workflow/packages/checkout.test.ts`.
...
- Do not parallelize tasks that edit `packages/riela/src/workflow/packages/checkout.ts`.
...
bun run typecheck
bun run packages/riela/src/bin.ts package install codex-impl-plan-completion-loop --registry default --output json
bun run packages/riela/src/bin.ts package install codex-recent-change-quality-loop --registry default --output json
bun run packages/riela/src/bin.ts package install codex-refactoring-divide-and-conquer --registry default --output json
...
bun run packages/riela/src/bin.ts workflow run <package-id> --from-registry --mock-scenario <fixture> --output json
git diff --check
```
...
PATH or in `node_modules/.bin`; `bun test packages/riela/src/cli.test.ts`
...
`packages/riela/src/workflow/packages/manifest.ts` and type exposure in
`packages/riela/src/workflow/packages/types.ts`. Added dependency-aware
persistent install in `packages/riela/src/workflow/packages/checkout.ts`
...
passed with
`bun test packages/riela/src/workflow/catalog.test.ts packages/riela/src/workflow/checkout/checkout.test.ts`.
...
helpers from `packages/riela/src/workflow/packages/checkout.ts` into
`packages/riela/src/workflow/packages/dependencies.ts`, then exporting the
new module from `packages/riela/src/workflow/packages/index.ts`.
`checkout.ts` is now 871 lines and `dependencies.ts` is 416 lines. Verification
passed with
`bun test packages/riela/src/workflow/packages/packages.test.ts packages/riela/src/workflow/packages/checkout.test.ts`,
`bun test packages/riela/src/workflow/catalog.test.ts packages/riela/src/workflow/checkout/checkout.test.ts`,
`bun test packages/riela/src/cli.test.ts -t "package commands search and install package ids"`,
...
`bun run packages/riela/src/bin.ts --user-root <temp> --project-root <temp> workflow run codex-impl-plan-completion-loop --from-registry --registry default --mock-scenario <repo-root>-packages/packages/codex-impl-plan-completion-loop/workflows/codex-impl-plan-completion-loop/mock-scenario.json --output json`.
...
`bun test packages/riela/src/workflow/packages/packages.test.ts packages/riela/src/workflow/packages/checkout.test.ts`.
Kept `bun test packages/riela/src/workflow/checkout/checkout.test.ts` as
