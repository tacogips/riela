...
private final class ManagedClaudeCodeProcess: @unchecked Sendable {
...
public enum ClaudeCodeAgentSDK {
  ...
  public struct Request: Equatable, Sendable {
    ...
    public init(prompt: String? = nil, sessionId: String? = nil, options: ClaudeCodeProcessOptions = ClaudeCodeProcessOptions(), streamMode: StreamMode = .raw) {
      self.prompt = prompt
  ...
  public static func runAgent<Runner: ClaudeCodeSessionRunner>(request: Request, runner: Runner) throws -> [ClaudeCodeAgentNormalizedEvent] {
    ...
    do {
      if let sessionId = request.sessionId {
        ...
        session = try runner.startSession(config: ClaudeCodeSessionConfig(prompt: request.prompt ?? "", options: request.options))
      }
    } catch {
      return [
    ...
    let started = ClaudeCodeAgentNormalizedEvent(type: "session.started", sessionId: resolvedSessionId, payload: ["resumed": .bool(resumed)])
    ...
    if request.streamMode == .raw {
      return [started] + messages.flatMap { line -> [ClaudeCodeAgentNormalizedEvent] in
    ...
    let normalized = messages.flatMap { line -> [ClaudeCodeAgentNormalizedEvent] in
  ...
  private static func rawMessageEvent(from line: ClaudeCodeRolloutLine, sessionId: String) -> ClaudeCodeAgentNormalizedEvent {
    ClaudeCodeAgentNormalizedEvent(type: "session.message", sessionId: sessionId, payload: ["chunk": rawLineValue(from: line)])
  }

  private static func rolloutErrorEvent(from line: ClaudeCodeRolloutLine, sessionId: String) -> ClaudeCodeAgentNormalizedEvent? {
    guard
      line.type == "event_msg",
    ...
    return ClaudeCodeAgentNormalizedEvent(type: "session.error", sessionId: sessionId, payload: ["message": .string(stringValue(object["message"]) ?? "Unknown rollout error")])
  }

  private static func completedEvent(result: ClaudeCodeSessionResult, sessionId: String, normalized: Bool) -> ClaudeCodeAgentNormalizedEvent {
    if normalized {
      return ClaudeCodeAgentNormalizedEvent(
    ...
    return ClaudeCodeAgentNormalizedEvent(type: "session.completed", sessionId: sessionId, payload: ["result": .object(sessionResultJSON(result))])
  }

  private static func sessionResultJSON(_ result: ClaudeCodeSessionResult) -> JSONObject {
    [
      "success": .bool(result.success),
      "exitCode": .number(Double(result.exitCode)),
      "stats": .object([
  ...
  private static func rawLineValue(from line: ClaudeCodeRolloutLine) -> JSONValue {
...
private final class ClaudeCodeResumeRolloutCapture: @unchecked Sendable {
  private let lock = NSLock()
  private let sessionId: String
  private let claudeCodeHome: String?
  private let watcher: ClaudeCodeRolloutWatcher
  private var attachedPath: String?
  private var closed = false

  init(sessionId: String, claudeCodeHome: String?, initialSession: ClaudeCodeSession?) {
    self.sessionId = sessionId
    self.claudeCodeHome = claudeCodeHome
    self.watcher = ClaudeCodeRolloutWatcher()
    if let initialSession {
      ...
      watcher.watchSessionsDirectory(path: ClaudeCodeRolloutWatcher.sessionsWatchDir(claudeCodeHome: claudeCodeHome))
    }
  }

  func flush(includeFinalBackfill: Bool) -> [ClaudeCodeRolloutLine] {
    lock.lock()
    guard !closed else {
      lock.unlock()
      return []
    ...
    var lines: [ClaudeCodeRolloutLine] = []
  ...
  private func finalBackfill() -> [ClaudeCodeRolloutLine] {
    let path: String?
    lock.lock()
    path = attachedPath
    lock.unlock()
    if let path {
      return (try? ClaudeCodeRolloutReader.readRollout(path: path)) ?? []
    }
...
private func rolloutPath(_ path: String, matchesSessionId sessionId: String) -> Bool {
  ...
  guard let first = try? ClaudeCodeRolloutReader.readRollout(path: path).first else {
    return false
  }
...
private extension Array where Element == ClaudeCodeRolloutLine {
  func deduplicatingStableRolloutLines() -> [ClaudeCodeRolloutLine] {
    var seen = Set<String>()
    var lines: [ClaudeCodeRolloutLine] = []
    for line in self {
  ...
  func streamChunks(granularity: String, sessionId: String) -> [ClaudeCodeRolloutLine] {
    guard granularity == "char" else {
      return self
    }
    return flatMap { line -> [ClaudeCodeRolloutLine] in
...
private extension ClaudeCodeRolloutLine {
