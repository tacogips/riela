...
validation, user-scope managed skill storage under `~/.riela-managed`, safe
...
- [x] Resolve user managed root to
      `~/.riela-managed/packages/<package-id>/<version-or-ref>/skills/`.
...
- [x] Store package records under
      `~/.riela/workflow-registry/checkouts/<install-id>.json`, with
      ...
      workflow name.
- [x] Write workflow-local `.riela-package-provenance.json` with
...
- [x] Support `riela cli workflow checkout <package-id>` package targets
      without breaking direct URL checkout.
- [x] Preserve package command compatibility for
      `riela cli workflow package checkout <package-id>` if still exposed.
- [x] Implement `riela cli workflow package status <workflow-name>` lookup
      for package records only, with `--scope project|user`,
      `--install-id`, text output, and JSON output.
- [x] Implement `riela cli workflow package update <workflow-name>` as a
...
- [x] Add a focused verification or refactor task for
      `packages/riela/src/cli/argument-parser.ts` so the known Biome
...
|--------|-----------|--------|-------|
| Package skill public types | `packages/riela/src/workflow/packages/types.ts` | Completed | Unit |
| Package layout and skill validation | `packages/riela/src/workflow/packages/skills.ts` | Completed | Unit |
| Package checksum and manifest inputs | `packages/riela/src/workflow/packages/checksum.ts`, `packages/riela/src/workflow/packages/manifest.ts` | Completed | Unit |
| Managed skill installation and projection | `packages/riela/src/workflow/packages/skill-install.ts` | Completed | Unit |
| Atomic package checkout | `packages/riela/src/workflow/packages/checkout.ts` | Completed | Unit/Integration |
| Provenance and update detection | `packages/riela/src/workflow/checkout/registry.ts`, `packages/riela/src/workflow/packages/checkout.ts` | Completed | Unit/Integration |
| CLI checkout/status/update flags and output | `packages/riela/src/cli/argument-parser.ts`, `packages/riela/src/cli/workflow-package-command-handler.ts` | Completed | CLI |
| Documentation and fixtures | `README.md`, tests | Completed | Regression |
| CLI parser size verification | `packages/riela/src/cli/argument-parser.ts` or extracted helper module | Completed | Biome |
...
- [x] Checkout rejects any skill vendor outside `agents`, `claude`, `codex`,
      `cursor`, and `gemini`.
- [x] User-scope skills are copied under `~/.riela-managed` and projected
...
bun run typecheck
bunx biome check packages/riela/src/workflow/packages packages/riela/src/workflow/checkout packages/riela/src/cli
